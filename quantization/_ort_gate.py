"""_ort_gate.py — gating test: yolo26n ONNX -> ORT quantize_static (klasik default) -> ultralytics val (CPU mAP)."""
...
def prep(p):
    im = lb(image=cv2.imread(p))                       # HWC BGR uint8 640
...
quant_pre_process(onnx_fp, pre_fp, skip_symbolic_shape=True)   # skip symbolic shape (puca na TopK/NMS), zadrži graph-opt
...
quantize_static(pre_fp, out_fp, Reader(calib, in_name), per_channel=True,
                ...
                calibrate_method=CalibrationMethod.Entropy)   # entropy ranges (odsiječe outliere) — i dalje klasik, bez bias-corr.

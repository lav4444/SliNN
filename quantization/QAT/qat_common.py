...
class QConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.conv2d(self.fq_in(x), wq, self.c.bias, self.c.stride,
                        self.c.padding, self.c.dilation, self.c.groups)
...
class QLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        s = self.w_scale if self.frozen else _wscale(w)
        wq = torch.fake_quantize_per_channel_affine(w, s, self.w_zp, 0, QMIN, QMAX)
        return F.linear(self.fq_in(x), wq, self.l.bias)
    ...
    for parent in model.modules():
        ...
        for (n1, c1), (n2, c2) in zip(ch, ch[1:]):
            if (isinstance(c1, nn.Conv2d) and isinstance(c2, nn.modules.batchnorm._BatchNorm)
                    and c2.num_features == c1.out_channels and c1.groups <= c1.in_channels):
                setattr(parent, n1, fuse_conv_bn_eval(c1.eval(), c2.eval()))
...
def wrap_model(model, skip=(), _prefix=""):
    ...
    for name, child in list(model.named_children()):
        ...
        if isinstance(child, nn.Conv2d):
            setattr(model, name, QConv2d(child)); n += 1

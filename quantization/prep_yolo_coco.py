"""
prep_yolo_coco.py — pripremi COCO-indeksirani pogled na 6-klasni sub10k dataset, da COCO-80 yolo
(.pt/.engine) možemo ispravno VALIDIRATI ultralyticsom (predikcije su COCO-80, pa i GT mora biti COCO-idx).

Radi NOVI dir `quantization/yolo_coco_data/` sa:
  images/<split>  -> symlink na originalne slike,
  labels/<split>  -> remapirane .txt (prvi token: 6-idx -> COCO-idx),
  yolo_coco.yaml  -> path + train/val/test + names = punih 80 COCO imena (iz modela).

Mapiranje (morphology COCO_YOLO_IDS): Person0->0, Car1->2, Truck2->7, Bus3->5, Motorcycle4->3, Bicycle5->1.
Originalni dataset se NE dira. Idempotentno (može se ponovo pokrenuti).
"""
...
DS2COCO = {0: 0, 1: 2, 2: 7, 3: 5, 4: 3, 5: 1}     # 6-idx -> COCO-80 idx
...
def remap_labels():
    ...
    for sp in SPLITS:
        ...
        if os.path.islink(dst_img):                    # ukloni stari (loš) directory-symlink
            os.unlink(dst_img)
        os.makedirs(dst_img, exist_ok=True)
        os.makedirs(dst_lbl, exist_ok=True)
        # slike: HARDLINK po datoteci (realpath ostaje u DST -> labele se razriješe na DST/labels, ne SRC)
...
def write_yaml():
    names = YOLO("/home/tomi/code/dipl/baseline_models/yolo26n/yolo26n.pt").names  # 80 COCO imena

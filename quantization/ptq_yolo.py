"""
ptq_yolo.py — PTQ za yolo26n i yolo26l (detekcija), POJEDNOSTAVLJENA matrica (svaka ćelija popunjena).

Redovi × {CPU, GPU}:
  FP32         CPU + GPU   (PyTorch)
  FP16         CPU + GPU   (PyTorch; CPU = sirovi .half(), spor — namjerno, pokazuje važnost HW podrške)
  INT8-static  CPU         (OpenVINO INT8, NNCF kalibracija; VNNI)         -> GPU ćelija = TRT (donji red)
  INT8-TRT     GPU         (TensorRT INT8, kalibracija)                    -> CPU ćelija = static (gornji red)

Kvaliteta = ultralytics val (mAP) na COCO-remapiranom yaml-u (prep_yolo_coco.py).
Latencija: PyTorch FP32/FP16 = sirovi forward benchmark (quant_common.benchmark, batch=1, warmup+medijan, CUDA sync);
           INT8 (TRT/OpenVINO) = native val speed['inference'] ms/img.
Veličina: .pt (FP32) / .pt÷2 (FP16) / OpenVINO dir / .engine (napomena: nisu samo težine).
BEZ CLI argumenata. Treba prethodno pokrenut prep_yolo_coco.py.
"""
...
def run_model(tag, weights, outdir):
    ...
    torch_model = YOLO(weights).model.float().eval()      # DetectionModel za sirovu latenciju

    # ---- FP32 (kvaliteta gpu val; latencije sirove) ----
    ...
    # ---- FP16 (kvaliteta gpu val half; CPU sirovi half = spor, par iter.) ----
    ...
    # ---- INT8-static (CPU, OpenVINO INT8 + NNCF kalibracija) ----
    try:

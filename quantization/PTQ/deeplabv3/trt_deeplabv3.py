"""
trt_deeplabv3.py — TensorRT INT8 (+ FP16) za DeepLabV3-MobileNetV3; popunjava GPU redke tablice.

Tok: DeepLabV3 -> ONNX (batch 1, fiksni 520x520) -> TensorRT engine
  * FP16-TRT: TRT half,
  * INT8-TRT: vlastiti IInt8EntropyCalibrator2 (kalibracija na VOC train podskupu).

ZAŠTO JE OVAJ REDAK VAŽAN: fasterrcnn ima INT8-TRT kao N/A jer je frcnn ONNX->TRT netrivijalan
(RPN/NMS/dinamični oblici). DeepLabV3 dijeli ISTI backbone (mobilenet_v3_large), ali nema tih
operacija. Ako ovdje prođe, frcnn-ov N/A prestaje biti neobjašnjena rupa i postaje tvrdnja:
pao je zbog DETEKCIJSKIH OPERACIJA, ne zbog obitelji modela.

KVADRATNI ULAZ I POŠTENA USPOREDBA:
  torchvision preset radi resize KRAĆEG RUBA na 520 -> svaka slika ima drugi oblik. TRT engine je
  građen na FIKSNIH 520x520, pa se za TRT redke slike resizeaju u kvadrat. To samo po sebi mijenja
  mIoU (mijenja se omjer stranica), pa se DODAJE i redak `FP32-sq520`: isti PyTorch model kroz ISTI
  kvadratni put. TRT redci se uspoređuju s NJIM, ne s originalnim FP32 retkom.

Device-buferi su TORCH CUDA tenzori (TRT-u treba samo data_ptr() + stream) -> bez cuda-python.
Pokrenuti NAKON ptq_deeplabv3.py. BEZ CLI argumenata.
"""
...
import ptq_deeplabv3 as PT                      # reuse: load_fp32, D, EB, Q, CSV_COLS, OUT_*, konstante
...
TRT_CALIB_IMAGES = 128                          # slika za INT8 kalibraciju (batch 1)
TRT_WORKSPACE_GB = 4
...
class OutOnly(nn.Module):
    """DeepLabV3 vraća OrderedDict {'out','aux'}; ONNX/TRT put treba JEDAN tenzor."""
    def __init__(self, m):
        super().__init__()
        self.m = m
...
def _preset():
    """mean/std iz samog presetа (fallback na ImageNet) -> kvadratni put koristi ISTU normalizaciju."""
    tf = D.transform()
...
def square_tensor(img, mean, std):
    """PIL -> [1,3,IMG,IMG] float32, resize u KVADRAT + ista normalizacija kao preset."""
...
class TRTRunner:
    ...
    def infer(self, x):                          # torch [1,3,IMG,IMG] cpu -> torch [1,C,h,w] cpu
        self.in_t.copy_(x.to("cuda", torch.float32))
        self._enqueue(); torch.cuda.synchronize()
        return self.out_t.detach().cpu().clone()

    def preload_for_latency(self, x):            # ulaz već na uređaju -> mjeri se samo execute
        self.in_t.copy_(x.to("cuda", torch.float32))
...
def confusion_square(infer, mean, std):
    """Konfuzijska matrica kroz KVADRATNI put (isti za TRT i za FP32-sq referencu)."""
...
def main():
    ...
    # ---- referenca: isti PyTorch FP32 kroz ISTI kvadratni put (da TRT ima s čim biti uspoređen) ----
    ...
    # ---- kalibracijske slike (VOC train, kvadratni put) ----
    ds_tr = D.voc("train")

...
def _yolo_student_signals(student, td, imgs):
    """Studentovi yolo KD-signali: gusti decode (xywh norm + probs nasih 6 + neck feat).

    BN OSTAJE NA RUNNING-STATS (). Do 6.12 je ovdje stajalo True ("BN se adaptira"),
    naslijedjeno iz morphology. Posljedica: loss se racuna pod BATCH statistikama, a mAP se mjeri pod
    RUNNING statistikama — dva razlicita rezima. Uz stari FT od 6 koraka to se jedva vidjelo; cim je
    Faza 1 uvela PUNE epohe, postalo je fatalno. Izmjereno na yolo26n (3 epohe):
        train_bn=True   KD 0.90 -> 0.51   mAP 0.3847 -> 0.3091   (loss pada, kvaliteta se rusi)
        train_bn=False  KD 0.24 -> 0.32   mAP 0.4086 -> 0.3909   (drzi; 2 uspjesna reza uz tol 97%)
    Zamrzavanje je i sigurno: tp reze BN running_mean/var zajedno s kanalima, pa preostale statistike
    ostaju ispravne. [[bn-eval-detection-trainmode]]"""

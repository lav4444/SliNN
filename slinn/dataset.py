"""
dataset.py — DatasetProbe: iz PATH-a na disku detektira FORMAT zapisa (i uzorak oznaka), model-agnosticno.

Treci sloj auto-detekcije (uz model-probe i label duck-typing): on-disk -> (format, task_hint, oznake).
Njusi STRUKTURU foldera + SADRZAJ uzorka label-datoteke. Format = sto je na disku; stvarni task odredi
task.py krizanjem s modelom. Neprepoznato -> unknown_format (glasno, ne pogadja). Katalog: SUPPORTED_DATASET_FORMATS.json.

Kaskada (najspecifičnije prvo): coco -> voc -> yolo(images+labels, peek retka) -> seg_masks -> folder_per_class
-> tabular -> nlp -> unknown_format. Wrapper-folderi (VOCdevkit/VOC2012, SpeechCommands/xxx) se preskaču.
"""
import glob
import json
import os
...
# kategorije za agnostic_sniffer; text je NAMJERNO dvosmislen (cesto oznaka, ne ulaz)
_CATS = {"image": _IMG, "audio": _AUD, "video": _VID, "tabular": _TAB, "text": _TXT}
_INPUT_CATS = ("image", "audio", "video", "tabular")   # jaki ULAZNI signali
...
def _has_ext(p, exts, depth=2):
    """Postoji li ijedna datoteka s danom ekstenzijom unutar `depth` razina."""
...
def _descend(p):
    """Slijedi lanac jedinstvenih podfoldera (VOCdevkit/VOC2012, SpeechCommands/speech_commands_v0.02),
    ali stani cim folder ima medij ili vise 'sadrzajnih' podfoldera."""
    for _ in range(5):
        subs = _dirs(p)
...
def _find_splits(p):
    """Dir-bazirani splitovi na korijenu ILI unutar images/ (yolo) / JPEGImages/. Txt-liste (VOC ImageSets,
    SpeechCommands) se ne hvataju ovime -> []."""
    found = []
...
# =========================== KLASIFIKATORI FORMATA =========================== #
def _is_hf_datasets(p):
    """HF `datasets` cache: dataset_info.json (+ *.arrow), obicno pod datasets/<name>/<config>/<ver>/<hash>/.
    task_hint iz sheme: ClassLabel feature -> classification; Sequence[ClassLabel] -> token (nepodrzano)."""
    info = None
...
def _peek_yolo_label(txt):
    """Vrati broj tokena po retku (mod) ili None. 5 = box (cls cx cy w h)."""
    try:
        for line in open(txt, encoding="utf-8").read().splitlines():
            parts = line.split()
            if parts:
                [float(x) for x in parts]                       # sve numericko?
                return len(parts)
    except BaseException:
        pass
    return None
...
def _is_folder_per_class(p):
    """Podfolderi = klase, svaki s medijem. Toleriraj par ne-klasnih ulaza (LICENSE, README, _background_)."""
    subs = _dirs(p)
...
# --- CONTENT PEEK (mjerenje, ne ekstenzija/ime) --- #
def _isnum(t):
    try:
        float(t)
        return True
...
def _img_role(path):
    """foto vs maska po SADRZAJU (generalizirajuce): palette/malo-vrijednosti = maska, inace foto."""
    try:
        from PIL import Image
        import numpy as np
        im = Image.open(path)
        if im.mode == "P":
            return "mask"                                      # paleta -> segmentacijska maska
        if im.mode in ("L", "I", "I;16", "F", "1"):
            a = np.asarray(im).ravel()
            u = np.unique(a[:: max(1, a.size // 4096)])         # subsample radi brzine
            return "mask" if u.size <= 64 else "photo"         # malo vrijednosti -> klase/labela
        return "photo"                                         # RGB/RGBA
    except BaseException:
        return None


def _peek_text_kind(path):
    """Peek par redaka -> vrsta oznake iz SADRZAJA (ne ekstenzije). Binarno (npr. .pt/.npy cache) -> None."""
    try:
        raw = open(path, "rb").read(4000)
    except BaseException:
        return None
    if b"\x00" in raw:
        return None                                            # null-bajt -> binarno, nije tekstualna anotacija
    printable = sum(1 for b in raw if 9 <= b <= 13 or 32 <= b <= 126)
    if raw and printable / len(raw) < 0.85:
        return None                                            # premalo ispisivih -> binarno
    ...
    if lines and all(all(_isnum(t) for t in ln) for ln in lines):
        ...
        if w == 1:
            return "scalar"
        # box-like: klasa-int + >=4 koordinate (5=GT 'cls cx cy w h', 6=+conf predikcija, 7=+track)
...
def _peek_table(path):
    """Otvori npz/parquet/csv i pogledaj STUPCE -> {label: bool, label_kind, input_kind}. Rjesava in-file labele."""
    ...
    try:
        ...
        if ext in (".csv", ".tsv"):
            ...
            lab = [n for n in names if any(t in n for t in LAB)]
            return {"label": True, "label_kind": None, "input_kind": "tabular"}  # csv gotovo uvijek ima target-stupac
    except BaseException:
        pass
    return {"label": False, "label_kind": None, "input_kind": "tabular"}


# --- STRATEGIJE OZNAKA (content + struktura) --- #
def _sniff_labels(path, samples, files):
    """Best-effort OZNAKE (generalizirajuce): stem-sidecar (peek sadrzaj) -> folder=klasa (samo ako DISJUNKTNI
    stemovi, ne paralelni view-i) -> manifest -> none. Vrati (method, task_hint, why)."""
    from collections import Counter
    sset = set(samples)
    stems = [_stem(s) for s in samples]
    sstemset = set(stems)

    # A) stem-match sidecar (peek SADRZAJ za tip) — provjeri PRIJE class_subfolder
    by_stem = {}
    for f in files:
        if f not in sset:
    ...
    if sstemset and len(matched) >= max(1, len(sstemset) // 2):
        ...
        kinds, peeked = Counter(), 0                            # peek preskace PRAZNE (npr. background bez okvira)
        for st in matched:
            if peeked >= 30:
                break
            for f in by_stem[st]:
    ...
    # B) folder = klasa — SAMO ako parent-dirovi imaju DISJUNKTNE stemove (inace su paralelni view-i)
    parents = {}
    for s in samples:
    ...
    if len(uparents) >= 2:
        ...
        # parallel view-i (npr. images/ + masks/) = malo dirova + skoro-savrseno 1:1 poravnanje stemova.
        # Koincidentalno preklapanje (npr. SpeechCommands: isti <govornik>_nohash u vise klasa) NIJE parallel.
...
def _sniff_image(path, files, imgs):
    """Slike: content-peek odvaja FOTO (ulaz) od MASKI (labela) po dirovima, pa odlucuje segmentation/detection/..."""
    ...
    inputs = photos or imgs                                    # ako nema jasnih foto-dirova -> sve su ulazi
    # 1) maske poravnate s fotama (dijele stemove) -> per-piksel labela (segmentation)
    if masks and inputs:
    ...
    # 2) inace: strukturne strategije nad FOTAMA (sidecar sadrzaj / folder-klase)
...
def _sniff_tabular(path, files, tabs):
    """Tabular kontejneri: otvori i pogledaj STUPCE -> in-file label + pravi input-modalitet (npr. image-u-parquetu)."""
    from collections import Counter
...
def agnostic_sniffer(path, cap=2_000_000):
    """FALLBACK kad kaskada ne prepozna format. Generalizirajuce + CONTENT-AWARE, egzaustivno (bez limita dubine):
      1) ULAZI = dominantna prava-media kategorija (image/audio/video/tabular; text dvosmislen).
      2) CONTENT-PEEK: slike -> foto vs maska po sadrzaju; tabular -> stupci (in-file label + pravi input tip).
      3) OZNAKE strukturno: stem-sidecar (peek) -> folder=klasa (samo disjunktni stemovi) -> manifest -> none.
    KLJUC: ulaz i oznaka ODVOJENI -> i bez oznaka (method='none') ostaju ULAZI za KD feat+logit."""
    from collections import Counter
    ...
    method, task_hint, why = _sniff_labels(path, cat_files, files)   # audio/video/text
    return _mk(input_cat, task_hint, method, cat_files, path, why)


# =========================== DETEKCIJA =========================== #
def detect_format(path):
    """Vrati dict: format, task_hint, splits, why. Neprepoznato -> unknown_format (nikad iznimka)."""
    ...
    for name, fn, args in (
        ...
        if r:
            ...
            return {"format": name, "task_hint": task_hint, "splits": _find_splits(root) or _find_splits(path),
                    "why": why, "root": root}
    sniff = agnostic_sniffer(root)                             # FALLBACK: generalizirajuci sniffer
    if sniff:
        return sniff
...
# =========================== UZORAK OZNAKA (za task.py) =========================== #
def label_sample(path, n=8):
    """Vrati uzorak oznaka u memoriji (kanonski oblik koji _label_signature razumije) ILI None.
    'todo' formati dizu NotImplementedError s jasnom porukom (glasno, ne tiho)."""
    import torch
    ...
    if fmt == "sniffed":
        ...
        return None                                            # ulazi nadjeni, oznake ne ovom metodom -> KD-only
    ...
    if fmt == "yolo":
        ...
        return out                                              # lista [N,5] -> _label_signature: detection
    ...
    if fmt == "voc":
        ...
        if masks:
            ...
            return torch.stack([x[:h, :w] for x in masks])     # [B,H,W] int -> segmentation
        return None
...
# =========================== SURVEY (egzaustivno brojanje + splitovi) =========================== #
def _survey(root, exts):
    """EGZAUSTIVNO (NE-pohlepno): prebroji SVE datoteke s `exts` po CIJELOM stablu, grupirano po splitu
    (najblizi predak-folder ciji naziv je split-ime). Ne staje na prvom nalazu — obidje sve foldere."""
    total, per = 0, {}
...
def hf_arrow_for(root, split=None):
    """Arrow datoteka trazenog splita. Split je u IMENU datoteke (`sst2-validation.arrow`), ne u
    folderu — zato ista normalizacija kao `_survey_hf` (`_SPLITS` redom, 'val' hvata i 'validation')."""
    arrows = _walk_files(root, (".arrow",), 100)
    if not arrows:
        return None
    if split:
...
def hf_tokenizer(root, model=None):
    """Tokenizer za `hf_datasets`, ISKLJUCIVO iz lokalnog cachea (`local_files_only`).

    Legacy je ovu stranu svjesno preskocio ("token bez tokenizera -> frozen random"): oznake su bile
    citljive, ulazi nisu, pa klasifikacija nije mogla dobiti pravu metriku. Model sam kaze koji mu
    tokenizer treba (`config.name_or_path`), a HF cache mapu imenuje `models--<org>--<ime>` — pa se
    pravi tokenizer bira sortiranjem kandidata po tom imenu. Bez toga bi se kod vise kesiranih
    modela uzeo prvi na koji se naidje."""
    try:
...
def hf_pairs(root, adapter, model, split=None, n=None):
    """(input_ids, class_idx) parovi iz HF arrow cachea. [] ako nema tokenizera ili stupaca.

    Oznaka: stupac tipa `ClassLabel` (fallback po imenu) — isti obrazac koji vec koristi citac oznaka
    za detekciju taska. Ulaz: prvi tekstualni stupac, tokeniziran na `adapter.imgsz` (token-mode
    ugovor je 1D int64 duljine imgsz)."""
    try:
        import datasets
        import torch as _t
    except BaseException:
...
def _survey_for(info):
    ...
    if fmt == "sniffed":
        ...
        if mod in ("image", "audio", "video"):
            ...
            if t > 0:
                return t, per                              # media kao datoteke
        return _survey_tabular(root)                        # podaci u kontejneru (parquet/npz/csv) ili tabular
    exts = {"voc": _IMG, "coco": _IMG, "yolo": _IMG, "seg_masks": _IMG,
            "folder_per_class": _MEDIA}.get(fmt, _MEDIA)
    return _survey(root, exts)


# =========================== FINALNI IZLAZ (ugovor za pipeline) =========================== #
def probe_dataset(path):
    """FINALNI izlaz DatasetProbe-a — jedinstveni ugovor za pipeline.
    Vrati dict s:
      samples_found  bool   ima li citljivih ULAZA
      labels_found   bool   mogu li se OZNAKE procitati (odvojeno od uzoraka)
      n_samples      int    egzaustivan ukupan broj uzoraka
      splits         {split: n} ili None
      mode           'full' (uzorci+oznake -> task detektor + enhanceri)
                     'core_kd_only' (samo uzorci -> general core KD: logit+feature)
                     'stop' (nista citljivo -> nema smisla dalje)
      format, modality, task_hint, why
    """
    info = detect_format(path)
    fmt = info["format"]
    ...
    return {"format": fmt, "modality": info.get("modality"), "task_hint": info.get("task_hint"),
            ...
            "_labels": labs,                                   # uzorak oznaka za task_detector (jedan izvor, bez glue-a)
...
def _tabular_target(root, n):
    ...
    if npz:
        ...
        if key is None:                                        # zadnji 1D niz kao cilj
...
# =========================== INPUT-READER (Faza 4.0: pravi ULAZI za KD) =========================== #
# Generickije od label-readera: medij se dekodira univerzalno (slika PIL, audio torchaudio, tabular numpy),
# usudjeno s adapter-ugovorom (mode/in_ch/size). Bez oznaka -> KD feat+logit svejedno radi (treba samo ulaze).
def _decode_image(f, in_ch, size):
    ...
    t = t.unsqueeze(0) if t.dim() == 2 else t.permute(2, 0, 1)   # [C,H,W]
    if t.shape[0] < in_ch:
...
def input_spec(*roots):
    """Ucitaj `input.json` — DEKLARIRAN ulazni ugovor modela. {} ako ga nema.

        {"length": 8000, "sample_rate": 8000, "in_ch": 1}

    Zasto konvencija a ne pogadjanje: duljina i frekvencija na kojoj je model treniran su odluke
    RECEPTA (M5: "downsample na 8 kHz -> [1, 8000]"). Iz tezina se ne vide, a fleksibilan model
    (potpuno konvolucijski, adaptivni pooling) prihvati bilo koju duljinu pa ga probe ne moze
    razlucti. Isti obrazac kao `classes.json`: podatak deklarira, kod ostaje agnostican."""
    seen = set()
    for r in roots:
...
def native_length(root, mode, in_ch, cap=1):
    """FALLBACK kad nema `input.json`: prirodna duljina IZ PODATAKA (dekodiraj stvarni uzorak).
    Bolje od "najvece radne s ljestvice" jer barem dolazi iz stvarnog signala. NE zna frekvenciju
    treninga — to samo deklaracija moze reci."""
    if mode != "seq":
        return None
    try:
        import torchaudio
...
def _decode_audio(f, in_ch, length, sr=None):
    """[in_ch, length] valni oblik. `sr` = frekvencija na kojoj je model TRENIRAN.

    Bez `sr` se snimka samo rezala/nadopunjavala — a datoteka je cesto na drugoj frekvenciji nego
    trening (SpeechCommands je 16 kHz, M5 je treniran na 8 kHz). Rezanje tada modelu da pola sekunde
    zvuka na dvostrukoj brzini. S `sr`: prvo se uzme ISJECAK ODGOVARAJUCEG TRAJANJA (length/sr sekundi
    pri izvornoj frekvenciji), pa se resamplira. Redoslijed je bitan — resampliranje 60-sekundne
    pozadinske snimke ravno na 8000 uzoraka trazi omjer 16000->133 Hz i gradi ogroman filtar
    (izmjereno: proces bude ubijen)."""
    import torch
    import torchaudio
    wav, fsr = torchaudio.load(f)                             # [C, L]
    ...
    if sr and fsr and fsr != sr:
        need = max(int(round(length * fsr / float(sr))), 1)   # koliko izvornih uzoraka = trazeno trajanje
        if wav.shape[1] > need:
            wav = wav[:, :need]
...
def _tabular_matrix(root, in_ch, split=None):
    """Ulazna matrica [N, in_ch] iz npz/csv. `split` bira `X_<split>` ako postoji.

    Do 6.13 je split bio ignoriran i vracao se PRVI 2D array koji odgovara sirini — a to je po
    redoslijedu kljuceva `X_train`. Mjerenje kvalitete tabularnog modela je time gledalo TRENING
    podatke. (`metric.pairs_regression` je bio ispravan jer sam trazi `X_<split>`.)"""
    import numpy as np
    for f in glob.glob(os.path.join(root, "**", "*.npz"), recursive=True)[:5]:
        try:
            z = np.load(f, allow_pickle=True)
            keys = list(z.files)
            if split and "X_{}".format(split) in keys:       # prioritet: eksplicitan split
...
_SPLIT_WARNED = set()                                        # da upozorenje ne ide na svaki poziv


def _media_files(root, exts, split, cap, skip_masks=False):
    """Do `cap` medijskih datoteka; preferiraj `split` u putanji; za slike preskoci MASKE (zelimo foto-ulaze)."""
    hit, rest = [], []
    for dp, _, files in os.walk(root):
    ...
    if split and not hit and rest and (root, split) not in _SPLIT_WARNED:
        _SPLIT_WARNED.add((root, split))                     # jednom po (root, split), ne po pozivu
...
def input_batch(path, adapter, device, split=None, n=8):
    """Vrati (batch, source): lista od n dekodiranih ULAZNIH uzoraka uskladjenih s adapter-ugovorom.
    source = 'image-files' | 'audio-files' | 'tabular' | 'fallback-random'. Fallback (probe-random) za
    slucajeve bez readera (token bez tokenizera, image-u-parquetu) -> KD i dalje radi (treba samo ulaze)."""
    import torch
...
# =========================== SPLIT POLITIKA (Faza 4.5a) =========================== #
def resolve_splits(splits):
    """Iz dostupnih dir-splitova odredi (train/val/test) izvore po politici:
      train+val+test -> as-is; train+val -> test=val; train+test -> val=test;
      samo train -> auto-carve val/test iz train; bez train / bez splitova -> pool sve + stratified 70/15/15.
    Vrati {train,val,test,method,note}; vrijednost je fizicki split-naziv ili 'AUTO' (treba carve)."""
...
def auto_carve(files, split_key, ratios=(0.70, 0.15, 0.15), seed=0):
    """Podijeli POPIS DATOTEKA na train/val/test kad dataset nema gotove splitove. Vrati trazeni dio.

    NE dira oznake — dijeli se redoslijed DATOTEKA. Grupni kljuc je ime NADFOLDERA:
      * `folder_per_class` (SpeechCommands, ImageFolder) -> nadfolder JE razred, pa je podjela
        STRATIFICIRANA: svaki split zadrzi isti omjer razreda.
      * sve datoteke u istom folderu -> jedna grupa -> obicna NASUMICNA (seedana) podjela.
    Degradacija je automatska, bez ijedne grane po tasku.

    Bez ovoga `AUTO` je znacio "cijelo stablo za SVAKI split", pa su se train i val poklapali —
    kvaliteta se mjerila na podacima na kojima se trenira."""
    if not files or split_key not in ("train", "val", "test"):
        return files
    import numpy as np
    files = sorted(files)                                    # deterministican redoslijed prije podjele
...
def stratified_split(labels, ratios=(0.70, 0.15, 0.15), seed=0):
    """Disjunktni (train,val,test) INDEKSI, deterministicno (seed). Stratifikacija po kljucu ovisnom o tasku:
      diskretno (int) -> po KLASI · kontinuirano (float) -> po KVANTILNIM BINOVIMA · multilabel/None -> RANDOM.
    Unutar svake grupe/klase dijeli po ratios -> jednaka zastupljenost klase u svakom splitu. Sample-level
    disjunktno (bez leakage); GRUPNI leakage (isti govornik/pacijent) NIJE pokriven bez grupnog kljuca."""
    import numpy as np
    if labels is None:
        keys = None
    else:
        ...
        if y.ndim > 1:                                       # multilabel/vektor -> stratifikacija ill-defined
            keys = np.zeros(len(y), dtype=int)
        elif np.issubdtype(y.dtype, np.floating):            # regresija -> kvantilni binovi
            edges = np.quantile(y, np.linspace(0, 1, 11))
            keys = np.clip(np.digitize(y, edges[1:-1]), 0, 9)
        else:                                                # klasifikacija -> po klasi
            keys = y.astype(int)

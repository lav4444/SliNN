...
TAP_CAP_ABS = 3          # TVRDA gornja granica broja tapova (odluka korisnika 2026-08: 3, prije 5)
TAP_CAP_FRAC = 0.10      # ...ili udio morphabilnih slojeva, STO JE MANJE (mali modeli -> 1-2 tapa)
# NAPOMENA: do 6.7 je ovdje stajao `max(ABS, FRAC*n)`, pa je ABS bio DONJA granica — veliki modeli su
# dobivali i 9+ tapova. Manje tapova = manje slojeva spregnutih s njima (v. `morph.tap_coupled`)
# = vise rezivog. Nusucinak: grublji feature-KD signal.
...
def pick_taps(cons, term, cls, shapes):
    ...
    cap = max(1, min(TAP_CAP_ABS, int(TAP_CAP_FRAC * n_morph)))

...
def pick_taps(cons, term, cls, shapes):
    ...
    if len(taps) > cap:
        order = {n: i for i, n in enumerate(cls)}
...
def positional(model, adapter, device, cls=None, shapes=None):
    ...
    meta = {"attention": len(att), "terminal": len(term), "terminal_names": sorted(term),
            "se": len(se), "taps": sorted(taps),
...
if __name__ == "__main__":
    ...
    run(os.path.join(_REPO, "baseline_models", "yolo26n", "yolo26n.pt"), "yolo26n", dev)
    run("fasterrcnn", "fasterrcnn", dev)
    run(os.path.join(_REPO, "pareto_sweep", "schoolcnn_pareto_final.pt"), "SchoolCNN", dev,
        code_dirs=[os.path.join(_REPO, "pruning", "critereum_experiment2")])

...
def kd_importance(student, teacher, adapter, batches, taps, kd_mode, out_kind, prunable=None, loss_fn=None, **w):
    ...
    import settings as _CFG                                  # citaj DINAMICKI (eksperiment ih override-a u runtimeu)
    ...
    try:
        for imgs in batches:
            ...
            for n, m, _, _ in leaves:
                ...
                if g is not None:
                    gd = g.detach().cpu()
                    # PRUNE rang: "grad" = mean|g|, "taylor" = mean|g·w| (w se tijekom prolaza NE mijenja
                    # - nema optimizer koraka - pa je mnozenje po batchu ekvivalentno mnozenju na kraju).
                    pv = (gd * m.weight.detach().cpu()) if mode == "taylor" else gd
                    acc[n] += pv.abs().flatten(1).mean(1)     # prune: po izlaznoj jedinici
                    gacc[n] += gd.reshape(gd.shape[0], -1)    # grow: signed grad matrica [O, in*k] (UVIJEK sirovi g)
            nb += 1
    finally:
    ...
    if norm in ("mean", "max"):                              # per-sloj skala -> globalni rang usporediv

"""_extractplug64.py -- 6.4 korak 3: izdvajanje DETEKCIJSKOG PLUGA iz morphology u
slinn/plugins/detection/.

Isti postupak kao _extract64.py (AST, izvorni rasponi, bez prepisivanja rukom).
Plug je JEDINI priznati per-obitelj dio: decode + NMS + mAP. Jezgra ga doseze kroz
jedan uzak otvor (backend.build_metric_fn); obrisi folder i jezgra i dalje radi
(pada na teacher-agreement gate).

OUT: slinn/plugins/detection/{adapters.py, profiles.py, dsconfig.py, __init__.py}
"""
...
# Redoslijed = redoslijed u analysis.py (ekstraktor ionako cuva izvorni poredak).
...
# ---------- 1) dsconfig.py: konstante KONKRETNOG detekcijskog dataseta ----------
...
# ---------- 2) profiles.py: cijeli modul (10/10 simbola su detekcijski) ----------
...
# ---------- 4) __init__.py: javni otvor pluga ----------

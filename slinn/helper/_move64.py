"""_move64.py -- 6.4 PRESELJENJE: sto tocno mora iz morphology/ u slinn/.

Racuna TRANZITIVNO ZATVORENJE krenuvsi od simbola koje slinn stvarno zove, pa mjeri
koliko od morphology doista seli, a koliko ostaje (mrtvo za nas -> ne seli).

Prati: top-level def/class I MODUL-RAZINSKE KONSTANTE (config.py je 100% konstanti,
profiles.PROFILES je rjecnik), te LOKALNE UVOZE u tijelima funkcija
(analysis.pick_adapter radi `import profiles` iznutra -> inace nevidljivo).

Nacelo (korisnik, 2026-08): arch_agnostic/slinn jezgra je smjer i UVIJEK pobjeduje.
Iz morphology seli SAMO ono cega u slinn/ nema. Duplikati -> KOLIZIJA, slinn pobjeduje.

OUT: ../REPORTS/move64.txt
"""
...
def parse(path):
    """-> (syms, aliases). syms ukljucuje def/class I top-level konstante."""
...
def imports_of(n):
    """Import/ImportFrom -> {alias_ili_ime: modul}. Hvata i `from X import y` (bare-name)."""
    out = {}
    if isinstance(n, ast.Import):
        ...
        for a in n.names:
            out["::" + (a.asname or a.name)] = n.module + "." + a.name   # bare-name -> puna kvalifikacija
    return out
...
def edges(mod, node):
    """Pozivi/reference iz simbola -> {'modul.ime'}. Ukljucuje LOKALNE uvoze u tijelu."""
    al = dict(ALIAS[mod])
    for x in ast.walk(node):                                  # lokalni uvozi (npr. pick_adapter: import profiles)
...
# ULAZNE TOCKE: sto slinn stvarno zove
...
for d in (SLINN, os.path.join(SLINN, "gui")):
    for f in sorted(os.listdir(d)):
        ...
        for al, sym in re.findall(r"\b(A|C|P|K|CFG)\.([A-Za-z_][A-Za-z0-9_]*)", txt):   # znamenke!
            mod = ALIAS2MOD[al]
...
# Klasifikacija po DOSEZNOSTI (ne po imenu): sto vise samo detekcijski ulaz doseze = plug.
...
# KOLIZIJE: simbol koji slinn VEC ima
slinn_syms = {}

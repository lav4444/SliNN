"""_cycle54.py — smoke za Fazu 5.4 (dead-removal + pun quality-gated ciklus) na voc_deeplabv3.
  full_cycle: dead-removal (Faza-1 ekviv.) -> KD-FT (referentni KD) -> morph_loop (prune+grow) s KD-proxy
  quality-gateom (gate = KD-loss vs teacher; GT NE u lossu) + best-model selekcija (najmanji GFLOPs pod kd_tol).
  Provjeri: dead-removal radi, ciklus napreduje, best-model odabran + forward OK + best.GFLOPs < baseline.
-> REPORTS/cycle54.txt
"""
import copy
import os
...
res = E.full_cycle(student, teacher, ad, dev, ctx, data, "voc_deeplabv3",
                   target_frac=0.15, ft_steps=5, dead_ft_steps=6, dead=True,   # dead=True: testiraj dead-removal put

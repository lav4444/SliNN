"""_agreegate.py — validacija TEACHER-AGREEMENT gate-a (Faza 5.6, unknown-task fallback).
full_cycle BEZ prave metrike -> auto agreement gate (slaganje s učiteljem na sirovim ulazima; kl=argmax, mse=R²).
Modeli: m5 (kl / argmax-agreement), midas (mse / R²-vs-teacher). -> REPORTS/agree_gate.txt
"""
import copy
import os
import sys
...
def run(name, spec, cd, data):
    ...
    res = E.full_cycle(student, teacher, ad, dev, ctx, data, name,
                       ...
                       imp_batches=3, max_steps=12, dead=False)   # BEZ metric_fn -> auto agreement gate
    g0 = res["g0"]
    ...
    return fwd                                                # gate-mehanizam radi ako da signal + forward-ok (čuvanje originala je VALJANO)

"""_prune52.py — smoke za Fazu 5.2 (prep + kontinuirana prune-petlja) na voc_deeplabv3 (2D, ima prostora).
  PREP:  install_sizing_shims (adapter-size, aktivan na import) + autobatch (siguran batch) na svim modelima.
  PETLJA: continuous_prune do ~12% ustede GFLOPs -> trajektorija (GFLOPs/params/KD po koraku), forward-safe,
          KD-FT oporavak izmedju koraka (teacher-cache). Provjeri: GFLOPs pada monotono, forward OK na kraju.
-> REPORTS/prune52.txt
"""
import copy
import os
...
# --- PREP: autobatch na svim modelima (dokaz da adapter-size shim + autobatch rade generalno) ---
PREP = [
...
for name, spec, cd, data in PREP:
    ...
    g = E.gflops(m, ad, dev)                                  # radi na svim modama (bio bi crash na hardkodu 640)
...
# --- PETLJA: kontinuirani prune na voc ---
...
res = E.morph_loop(student, teacher, ad, dev, ctx, data, "voc_deeplabv3",   # reinvest_frac=0 -> čisti prune (bez growa)

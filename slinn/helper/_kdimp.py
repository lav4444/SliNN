"""_kdimp.py — smoke za Fazu 4.3 (KD-grad vaznost). Za svaki model: teacher=smrznuta kopija, student=kopija+sum
(da KD>0, simulira razidjenost iz Faze 1/2); izracunaj kd_importance na PRUNABILNIM slojevima i provjeri da je
KONACNA + DISKRIMINIRA (var>0 -> smislen rang) + gavg (grow signal) ima oblik [O, in*k]. -> REPORTS/kd_imp.txt
"""
import copy
import os
import sys
...
for name, spec, cd, out_kind in MODELS:
    try:
        ...
        batches = [[pr._one(dev) for _ in range(4)] for _ in range(3)]   # 3 batcha x 4 uzorka
        ...
        nzero = sum(1 for v in imp.values() if float(v.abs().sum()) == 0)     # nul-grad = izvan KD-signala (prune-safe)
        ...
        # pokazi jedan sloj: raspon importance + koliko kanala
        ex = next(iter(imp))
        v = imp[ex]

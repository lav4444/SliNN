"""
sweep_cls.py — Pareto-sweep kompresije za SchoolCNN (KLASIFIKACIJA, multi-label "prisutnost").

Paralela yolo sweepu (sweep.py), ali za "velik model — lagan zadatak":
  * model:   SchoolCNN (~3.7M, 2.66 GFLOPs) iz pruning/critereum_experiment2
  * zadatak: multi-label klasifikacija (je li svaki od 6 objekata prisutan na slici)
  * podaci:  ISTI dataset (sub10k_open_images_v7), mete = prisutnost klase u YOLO labeli
  * metrika kvalitete: macro mAP na val skupu (common.evaluate -> "map")

NE koristi morphology (niti ga uvozi) — sav task-sloj dolazi iz experiment2:
  common.py (podaci/eval/benchmark) + pruning_lib2.py (strukturirani prune + FLOPs + BN) + model_cnn.

Tok (preskačemo bilo kakvu fazu 1 — krećemo od treniranog baseline-a):
  baseline (teacher = frozen original) ->
  PETLJA: PRUNE (ukloni ~PRUNE_STEP_FRAC params, KD-svjesna važnost) -> BN-rekalib ->
          FINE-TUNE (FT_EPOCHS_PER_STEP KD epoha, logit-destilacija od originala, BEZ GT) ->
          VALIDIRAJ macro mAP -> zapiši red u CSV
  dok je GFLOPs > MIN_GFLOPS_FRAC × original (ili dok ima što rezati, ili dok napredak ne presuši).

SAMO podrezivanje (bez grow). KD-only (bez GT) — dosljedno yolo sweepu.
...
# --- experiment2 na path (uvozimo ga, ali ga NE mijenjamo) ---
_HERE = os.path.dirname(os.path.abspath(__file__))
_EXP2 = os.path.join(os.path.dirname(_HERE), "pruning", "critereum_experiment2")
sys.path.insert(0, _EXP2)

import common                       # dataset (multi-label), evaluate (macro mAP/F1/acc), benchmark
import pruning_lib2 as P            # build_plan / compute_importance / select_kept_indices / apply_pruning / count_flops / recalibrate_bn
from model_cnn import SchoolCNN, INPUT_SIZE

# ============================================================================ #
#                          POSTAVKE SWEEPA (uredi ovdje)                         #
# ============================================================================ #
MODEL_TAG   = "schoolcnn"          # prefiks izlaznih datoteka (ne prebriše yolo26n_*)
CKPT_PATH   = os.path.join(_EXP2, "checkpoints", "best.pt")   # {"model": state_dict, "stats": ...}

PRUNE_STEP_FRAC   = 0.03           # ciljano OSLOBOĐENJE GFLOPs po koraku = % ORIGINALNIH GFLOPs (kao yolo sweep, budžet u GFLOPs)
PRUNE_LAYER_CAP   = 0.15           # max udio kanala JEDNOG sloja maknut u JEDNOM koraku (anti-kolaps; kao morphology)
QUALITY_METRIC    = "map"          # metrika kvalitete (zapisuje se i crta): "map" (macro mAP) | "f1" | "acc"
MIN_GFLOPS_FRAC   = 0.005           # POD: kad GFLOPs padne < ovaj % originala -> KRAJ
MIN_PROGRESS_FRAC = 0.0015         # EARLY-STOP: ako korak NETO oslobodi < ovaj % orig. GFLOPs -> KRAJ. 0 = isključeno.
FT_EPOCHS_PER_STEP = 2             # broj KD fine-tune epoha nakon SVAKOG reza (validacija tek nakon njih)

# Prune detalji
CRITERION   = "taylor"             # važnost: "magnitude" (bez backw.) | "gradient" | "taylor" | "hessian" (KD-svjesne)
MIN_KEEP    = 2                    # floor: min izlaznih jedinica po sloju (anti-kolaps)
CALIB_BATCHES = 4                  # batcheva za KD-svjesnu važnost (gradient/taylor/hessian)
MAX_STEPS   = 500                  # sigurnosna gornja granica

# Trening / KD
LR          = 1e-3 / 3.0           # Adam LR (kao experiment2 FT)
BATCH_SIZE  = 32
NUM_WORKERS = 4

# Vjernost / okruženje
DEV_SUBSET  = None                 # ⚠️ None = svi podaci. Postavi npr. 500 za brzi smoke-test (cap po splitu).

# Izlaz
OUT_CSV   = os.path.join(_HERE, f"{MODEL_TAG}_pareto_run.csv")
OUT_META  = os.path.join(_HERE, f"{MODEL_TAG}_pareto_run_meta.json")
OUT_MODEL = os.path.join(_HERE, f"{MODEL_TAG}_pareto_final.pt")   # samo ZADNJI (pruned) model
...
def _conv_out_hw(model, device):
    """Prostorna površina (H*W) izlaza svakog conv-a (preko jednog forwarda s hookom) — za FLOPs/jedinica."""
    hw, handles = {}, []
...
def select_prune_gflops(model, entries, by_name, importance, target_gflops,
    ...
    """GFLOPs-budžet selekcija (mirror yolo sweepa): makni NAJNEBITNIJE izlazne jedinice (globalni rang
    po važnosti, uzlazno) dok SPREGNUTI oslobođeni GFLOPs ne dosegnu target_gflops, uz per-layer cap i floor.
    SPREGNUTO = removing izlazne jedinice sloja L oslobađa: (a) FLOPs samog L + (b) FLOPs potrošača C kojem
    nestaje `block` ulaznih značajki (za conv5->fc1 block=flatten_hw). Vrati (kept_idx, est_freed_gflops)."""
    prunable = [e for e in entries if e["prune_out"]]
    out_hw = _conv_out_hw(model, device)
    cur_out = {e["name"]: e["orig_out"] for e in entries}            # build_plan na TRENUTNOM modelu -> orig_out = trenutna širina
    consumers = {}                                                  # name -> [(consumer_entry, block)]
    for c in entries:
    ...
    def per_unit_gflops(e):
        ...
        down = 0
        for c, block in consumers.get(e["name"], []):               # potrošač gubi `block` ulaznih značajki po maknutoj jedinici
    ...
    ranked.sort(key=lambda t: t[0])                                 # najnebitnije prvo
...
def run_sweep():
    ...
    # --- teacher = original (frozen); student = trenirabilna kopija ---
    ...
    # --- loaderi (isti dataset; DEV_SUBSET kapira po splitu) ---
    ...
    # fiksni calib skup za KD-svjesnu važnost (stabilnije rangiranje)
    ...
    # --- KD loss: učenik oponaša sigmoid-vjerojatnosti zamrznutog originala (BEZ GT) ---
    ...
    def kd_loss_fn(model, batch):                       # za compute_importance (vraća (loss, bs))
        x = batch[0].to(device, non_blocking=True)
        return kd_soft(model(x), x), x.size(0)

    def ft_kd(epochs):                                  # FT recovery: KD epoha(e), Adam + AMP
        student.train()
    ...
    target_gflops = PRUNE_STEP_FRAC * base_gflops          # rez po koraku = % ORIGINALNIH GFLOPs (kao yolo)
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        # BN-rekalibracija na svježoj (prorezanoj) arhitekturi prije FT
        P.recalibrate_bn(student, calib_batches, lambda b: b[0], device, reset=True)

        # ============================ FINE-TUNE (KD recovery) ============================ #

"""
plot_compare.py — preklopi VIŠE Pareto sweepova na jedan graf, NORMALIZIRANO, da se vidi
RAZLIKA U OBLIKU krivulje (teza iz pog. 2): "velik model — lagan zadatak" (ravna krivulja)
vs "slab model — težak zadatak" (strma krivulja).

Osi (normalizirane, pa su modeli različitih skala usporedivi):
  x = % originalnih GFLOPs (100% = baseline),  y = kvaliteta / baseline-kvaliteta (1.0 = baseline).

Čita parove (CSV, meta) po prefiksu iz RUNS. BEZ CLI argumenata.
"""
...
# (prefiks, prikazno ime, boja). Datoteke: <prefiks>_pareto_run.csv + <prefiks>_pareto_run_meta.json
RUNS = [
...
def main():
    ...
    for prefix, label, color in RUNS:
        ...
        ys = [p["q"] / base_q for p in pts]               # kvaliteta relativna na baseline
        ...
        ax.plot([p["pct"] for p in front], [p["q"] / base_q for p in front],
                "-", color=color, lw=2.6, alpha=1.0)       # istaknuta nedominirana fronta (deblje)
        plotted += 1
    ...
    ax.invert_xaxis()                                      # od 100% (original) prema jakoj kompresiji slijeva-nadesno
    fig.tight_layout()

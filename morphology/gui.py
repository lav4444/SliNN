"""
gui.py — morphology lokalni GUI (Streamlit). TANKI sloj nad analysis.py + compress.py
(sav compute je u logici; GUI samo renderira). Pokretanje:  streamlit run gui.py

Stranice:
  Overview        — analiza ucitane mreze (deep, s % loadingom) + highlights
  Compress        — postavke (placeholder) + dead/near-dead rez + FT recovery + live view
  About solution  — dinamicki popis podrzanog (capabilities)
"""
...
# Metrike koje korisnik moze birati za pracenje/early-stop (label -> torchmetrics kljuc). Default = config.FT_METRICS.
METRIC_OPTS = {"val mAP@[.5:.95]": "map", "val mAP@.50": "map_50", "val mAP@.75": "map_75", "val mAR@100": "mar_100"}
METRIC_NICE = {v: k for k, v in METRIC_OPTS.items()}                  # kljuc -> lijepi label (za kartice/grafove)
...
if config.DEV_DATA_SUBSET:                               # glasno upozorenje dok je dev-podskup aktivan (da se ne zaboravi)
...
def page_overview():
    ...
    if spec not in cache:                                # deep analiza UVIJEK (prvi put po modelu) + % loading
    ...
    perf = rep.get("perf")                               # izmjereno u analizi i spremljeno na disk; compress ovo CITA
    if perf:
        st.subheader("Performanse (mjereno — compress ovo čita kao baseline, bez ponovnog računanja)")
        val = perf["maps"].get("val", {})                # headline: dvije najinformativnije (val mAP + val recall)
        cp = st.columns(4)
        ...
        with st.expander("Detaljne performanse (sve metrike, po splitu)"):   # puni COCO set tek na zahtjev
            keys = []                                    # unija metrika svih splitova
...
def _pid_alive(pid):
    try:
        os.kill(int(pid), 0)                                 # signal 0 = samo provjera postojanja procesa
        return True
    except Exception:
        return False
...
def _read_status():
    ...
    if s.get("state") == "running" and s.get("pid") and not _pid_alive(s["pid"]):
        s = {"state": "error", "msg": "worker proces nije ziv (pao bez statusa)"}  # mrtav worker ne zakljucava UI
    return s


@st.cache_resource                                       # @cache_resource = izvrsi se TOCNO JEDNOM po pokretanju servera
def _startup_cleanup():
    """Pri svakom novom pokretanju web appa ocisti staru trajektoriju/status/log (osim ako worker stvarno jos radi),
    da Compress ne pokazuje grafove od proslog runa."""
    s = _read_status()
    ...
    if not alive and os.path.isdir(JOB):
        for f in ("trajectory.jsonl", "status.json", "worker.log"):
            try:
                ...
                pass
    ps = _read_prep_status()                              # prep: ocisti osim ako prep worker stvarno jos radi
...
def _read_prep_status():
    ...
    if s.get("state") == "preparing" and s.get("pid") and not _pid_alive(s["pid"]):
        s["state"] = "error"                              # prep proces pao bez statusa
    return s
...
@st.fragment(run_every=2)                                # osvjezava SAMO prep-korake dok priprema traje
def _prep_fragment():
    prep = _read_prep_status()
    _render_prep_steps(prep)
    if prep.get("state") != "preparing":                 # gotovo (ready/error) -> puni rerun (staticni prikaz)
        st.rerun()
...
def _render_running_view(cstatus):
    """Dok kompresija traje: sakrij pripremu/postavke, ostavi samo INFORMATIVNO (pracene metrike + Train until) + zivi prikaz."""
    cfg = {}
    ...
    if os.path.exists(cfgp):
        try:
            ...
            pass
    perf = _read_prep_status().get("steps", {}).get("perf", {}).get("data", {})   # pocetne (baseline) vrijednosti
    ...
    nice = ", ".join(f"{METRIC_NICE.get(m, m)} ({_f(m, _init.get(m, 0.0))})" for m in mets)   # metrika (početna)
    tf = config.FT_RECOVERY_FRAC                                                                # tolerancija = % originala
    ...
    if st.button("⏹ Zaustavi trening"):
        ...
        st.rerun()
    _live_fragment()                                     # grafovi/kartice/log (bez full-page reloada)
...
def page_compress():
    ...
    cstatus0 = _read_status()                            # kompresija u tijeku -> samo info + zivi prikaz (bez prep/postavki)
    ...
    if pstate in ("idle", "stopped"):                     # PRVI ulaz (ili nakon prekida) -> automatski pokreni pripremu
        _launch_prep()
        st.rerun()

    if pstate == "preparing":
        _prep_fragment()
        return                                            # dok priprema traje, ne prikazuj postavke/kompresiju
    ...
    # ===================== POSTAVKE + KOMPRESIJA (prep ready) ===================== #
    ...
    perf = prep.get("steps", {}).get("perf", {}).get("data", {})   # pocetne vrijednosti metrika (iz baseline-a)
    _init = {"gflops": perf.get("gflops", 0.0), "params": perf.get("params", 0.0),
             "map": perf.get("val_map", 0.0), "mar_100": perf.get("val_mar", 0.0)}

    def _fmt(k, v):                                       # formatiranje po metrici
        return f"{v/1e6:.2f} M" if k == "params" else f"{v:.3f}"

    # --- KARTICA 0: kvantizacija -> align M (semafor) ---
    ...
    # --- KARTICA 1: metrika kvalitete (semafor: 🟡 dok nije potvrđeno, 🟢 kad je) ---
    with st.container(border=True):
        h1 = st.empty()                                   # naslov sa semaforom (popuni se nakon citanja izbora)
    ...
    # --- KARTICA 2: uvjet zaustavljanja (semafor) ---
    with st.container(border=True):
        h2 = st.empty()
        p2_metric = st.selectbox("Train until", _p2keys, index=_p2idx)
        p2_key = PHASE2_OPTS[p2_metric]
        info_ph = st.empty()                              # ISPIS IZNAD slidera (puni se nakon citanja slidera)
    ...
    if start:                                             # (kompresija-u-tijeku se hvata na vrhu funkcije -> ovdje nije running)
        metrics = (["map"] if opt_map else []) + (["mar_100"] if opt_mar else [])
        batch = _read_prep_status().get("steps", {}).get("batch", {}).get("data", {})   # auto-batch iz prep probe (TRAIN/EVAL/GRAD)
        json.dump({"metrics": metrics, "align_m": align_m, "phase2": {"metric": p2_key, "frac": p2_frac / 100},
                   "batch": batch},
                  open(os.path.join(JOB, "config.json"), "w"))     # model fiksiran; precompute+baseline vec gotovi (prep)
    ...
    pts = _read_traj()                                    # zavrsen/zaustavljen prijasnji run -> prikazi rezultate
    if pts:
        st.divider()
...
def _trend_chart(pts, key, ylabel, color="#4c78a8", tol_val=None):
    """Altair linijski graf: y-raspon [min-buffer, max+buffer] + crvena isprekidana crta na POCETNOM (baseline) stanju.
    Ako je tol_val zadan -> i ZELENA crta na toj apsolutnoj vrijednosti: za kvalitetu = prag tolerancije (frac×original),
    za align_score = NAJNOVIJI dostizni max align. (st.line_chart ne podrzava custom y-domenu ni referentnu liniju.)"""
    import altair as alt
    import pandas as pd
    pairs = [(i, p.get(key)) for i, p in enumerate(pts) if p.get(key) is not None]   # preskoci tocke bez te metrike
    if not pairs:
    ...
    if tol_val is not None:                                            # prosiri domenu da zelena crta UVIJEK bude vidljiva
        lo, hi = min(lo, tol_val), max(hi, tol_val)
    span = hi - lo
    pad = span * 0.1 if span > 0 else (abs(hi) * 0.1 if hi else 1.0)   # buffer; degenerira li raspon, uzmi ~10% vrijednosti
    ...
    rule = alt.Chart(pd.DataFrame({"base": [base]})).mark_rule(
        color="#e45756", strokeDash=[6, 4], size=2).encode(y="base:Q")   # crveno = pocetno stanje
    chart = line + rule
    if tol_val is not None:
        tol_rule = alt.Chart(pd.DataFrame({"tol": [tol_val]})).mark_rule(
            color="#54a24b", strokeDash=[5, 3], size=2).encode(y="tol:Q")   # zeleno = tol (kvaliteta) ILI dostizni max (align)
        chart = chart + tol_rule
    return chart.properties(height=240, width="container")


def _render_live(pts, status):                               # crta metrike + grafove + log (jednom; bez full-page reloada)
    running = status.get("state") == "running"
    cfgp = os.path.join(JOB, "config.json")                  # koje su metrike odabrane za ovaj run
    mets = ["map"]
    if os.path.exists(cfgp):
        try:
    ...
    if pts:
        ...
        otag = lambda k: " *" if k in mets else ""           # * = metrika koja se optimizira
        r1 = st.columns(3)                                   # RED 1: kvaliteta + params
        ...
        r2 = st.columns(3)                                   # RED 2: sve oko GFLOPs
        ...
        r3 = st.columns(3)                                   # RED 3: velicina + poravnanje + (placeholder)
        ...
        # --- grafovi: po jedan za svaku headline metriku; korisnik bira checkboxima; 2 grafa po redu ---
        # default ON: pracena(e) metrika (sve ako ih je vise) + GFLOPs + params + align score.
        ...
        for i, (k, lbl) in enumerate(GRAPHABLE):
            has = any(p.get(k) is not None for p in pts)            # nudi samo metrike koje imaju podatke
            if cbcols[i % 4].checkbox(lbl, value=_def_on(k), key=f"g_{k}", disabled=not has) and has:
                chosen.append((k, lbl))
        for i in range(0, len(chosen), 2):                          # 2 stupca -> 2 grafa po redu; broj redaka varira
            gcols = st.columns(2)
            for j, (k, lbl) in enumerate(chosen[i:i + 2]):
                with gcols[j]:
                    st.markdown(f"**{lbl}**")
                    tol_val = None
                    if (k == "val_map" and "map" in mets) or (k == "val_mar" and "mar_100" in mets):   # kvaliteta: zelena = prag tolerancije
                        b = next((p.get(k) for p in pts if p.get(k) is not None), None)                # pts[0] = original baseline
                        tol_val = b * config.FT_RECOVERY_FRAC if b is not None else None
                    elif k == "align_score":                                                           # align: zelena = NAJNOVIJI dostizni max
    ...
    logp = os.path.join(JOB, "worker.log")                       # trening-ispis (isto sto bi islo u terminal)
    if os.path.exists(logp):
        with st.expander("Trening log", expanded=running):
            st.code(open(logp, errors="replace").read()[-6000:] or "(prazno)", language="text")


@st.fragment(run_every=2)                                    # osvjezava SAMO ovaj fragment (ne cijelu stranicu)
def _live_fragment():
    status = _read_status()
    pts = _read_traj()
    _render_live(pts, status)                                # uvijek crta (inace fragment ostane prazan); ako nema nove
    #                                                          tocke, vrijednosti su iste -> nema vidljivog treperenja
    if status.get("state") != "running":                     # gotovo -> puni rerun -> staticni prikaz (polling staje)
        st.rerun()
...
_startup_cleanup()                                       # jednom po pokretanju servera: makni stare grafove/log

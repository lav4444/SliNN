"""
introspect.py — AUTO-PREPOZNAVANJE karakteristika modela (task / komponente / vrste slojeva / KD-tapovi).
Cilj: pipeline radi PO-SVOJSTVU, ne po-modelu. profile(model) samo CITA model (ne mijenja ga) i vraca
strukturirani opis koji kasnije voze dijeljeni handleri (prune/BN/KD/metrika/postprocess).

Samo torch/nn ovisnost (bez torchvision/ultralytics importa) -> radi za bilo koji nn.Module.
"""
...
def _ultra_head(model):
    """Zadnji modul ultralytics modela (Detect/Segment/Classify/Pose) ili None."""
...
def profile(model):
    """Vrati opis modela: {module, task, family, num_classes, components, layers, kd_taps, notes}.
    KD-tapovi su zasad DEKLARATIVNI (koji distilacijski signali POSTOJE); pristupnici dolaze u profilima."""
    ...
    # --- dostupni KD-tapovi (po komponentama) ---
    taps = []
    if has_fpn:

"""
Evaluate YOLO26l (COCO-pretrained) on train, val, and test splits of the
sub10k Open Images V7 dataset, in one go.

Layout:
    <DATASET_ROOT>/yolo26l/<split>/soft/<stem>.pt    pre-NMS dense (8400 anchors x 6 classes)
    <DATASET_ROOT>/yolo26l/<split>/labels/<stem>.txt post-NMS hard pseudo-labels (YOLO + conf)
    <DATASET_ROOT>/yolo26l/meta.json                 global metadata + per-split summary
    <SCRIPT_DIR>/eval_result.txt                     mAP + speed for all three splits

Splits whose `<split>/soft/` already contains all expected `.pt` files are
skipped at inference time; metrics are recomputed from saved tensors instead.
Delete the soft/ folder for a split (or just one file inside it) to force
re-inference.
...
OURS_TO_COCO = [0, 2, 7, 5, 3, 1]  # ours -> COCO id
...
BENCHMARK_N_DISCARD = 2   # discard 2 slowest (assumed warmup)
BENCHMARK_SEED = 42

# Backbone feature extraction (for FGD-style KD feature distillation in KD_GT_FGD students)
# When enabled, each saved .pt file gets a new key "p3_features" holding the layer-4
# backbone output as fp16 ([512, 80, 80] for yolo26L). Adds ~55 GB across all splits.
EXTRACT_BACKBONE_FEATURES = True
BACKBONE_P3_LAYER_IDX = 4  # layer index of yolo26 backbone P3 stage output (after dark3 C3k2)
...
# WRITE_CACHE=0 -> ne generiraj KD kes (meke/tvrde oznake, meta.json)
# Bez oba: tocno postojece ponasanje.
...
SPLITS = tuple(s for s in SPLITS if EVAL_SPLITS is None or s in EVAL_SPLITS)
...
def letterbox_image(img_bgr, color=(114, 114, 114)):
    """Letterbox to exactly IMG_SIZE x IMG_SIZE (square). Matches Ultralytics' LetterBox."""
    h, w = img_bgr.shape[:2]
...
def nms_post_process(boxes_xywh: torch.Tensor, class_probs: torch.Tensor,
    ...
    """Class-aware NMS on letterbox-space dense outputs.

    Returns (boxes_xyxy_letter [M,4], scores [M], class_ids [M])."""
...
def is_split_complete(paths: dict) -> bool:
    ...
    if not (image_stems.issubset(soft_stems) and len(image_stems) > 0):
        return False
    # If extracting backbone features, also verify they're present in the saved .pt files.
    # Switching EXTRACT_BACKBONE_FEATURES False -> True (or upgrading a stale cache) triggers
    # re-inference for the whole split, which is what we want.
...
def run_inference_for_split(model: YOLO, split: str, paths: dict) -> dict:
    """Manual preprocessing + direct model.model() forward.

    Bypasses Ultralytics' predict() pipeline (which uses dynamic letterbox to
    multiples of 32, producing variable anchor counts per image). We force a
    fixed 640x640 letterbox so all images yield 8400 anchors and the saved
    letterbox metadata matches the input the model actually saw.
    """
    if WRITE_CACHE:
    ...
    # Optional: register a hook on the P3 backbone layer to capture intermediate
    # features for FGD-style KD. Stored as fp16 [512, 80, 80] per image.
    features_cache: dict = {}
    hook_handle = None
    if EXTRACT_BACKBONE_FEATURES:
    ...
    for img_path in img_paths:
        ...
        if isinstance(out, (tuple, list)):
            out = out[0]
        dense = out[0]                                        # [4+nc, anchors] on GPU
        n_classes = dense.shape[0] - 4
        if anchor_count is None:
            anchor_count = int(dense.shape[1])

        boxes_xywh = dense[:4].T.contiguous().cpu()           # [anchors, 4]
        class_probs_full = dense[4:4 + n_classes].T           # [anchors, 80]
        ...
        if EXTRACT_BACKBONE_FEATURES and "p3" in features_cache:
            p3_feat = features_cache["p3"][0].cpu().half()   # [512, 80, 80] fp16
        ...
        _soft_save(save_dict, paths["soft_dir"] / f"{stem}.pt")
    ...
    m = metric.compute()
    # Clean up the P3 hook before returning
...
def load_and_eval_split(split: str, paths: dict) -> dict:
    ...
    return {
        ...
        "anchor_count": anchor_count,
        "avg_inference_ms": sum_inf_ms / max(n, 1),
        "avg_total_ms": None,  # not measured when loading from disk
        "map": float(m["map"]),
        "map_50": float(m["map_50"]),
        "map_75": float(m["map_75"]),
        "mar_100": float(m["mar_100"]),
...
def benchmark_cpu_vs_gpu_latency(model: YOLO, train_img_dir: Path) -> dict:
    """Forward-pass latency on CPU then GPU for BENCHMARK_N_IMAGES random train images.

    Same images, same input tensors, same model weights — only device differs.
    Preprocessing is done once up-front so it doesn't pollute the timed window.
    Per-image times are reported; mean is over the (N - N_DISCARD) fastest
    (the slowest few are typically warmup / kernel-compile time).
    """
    ...
    print(f"\nBenchmark: preprocessing {BENCHMARK_N_IMAGES} random {SPLITS[0]} images...")
    inputs = []
    for img_path in sampled:
        tensor, _, _, _, _ = preprocess_image(img_path)
        inputs.append(tensor.unsqueeze(0))   # [1, 3, 640, 640]
...
def format_benchmark_results(results: dict) -> str:
    lines = [
        ...
        f"({BENCHMARK_N_IMAGES} random {SPLITS[0]} images, seed={BENCHMARK_SEED})",
        "=" * 64,
        "",
    ]
    for device_name in ("cpu", "cuda"):
...
def main():
    ...
    # Decide what needs inference
    plan = []
    for split in SPLITS:
        paths = split_paths(split)
        if is_split_complete(paths) and not EVAL_LIMIT:
    ...
    if needs_model:
        ...
        # YOLO26 ships with an end-to-end head that returns top-K detections;
        # flipping this back to the YOLOv8-style path makes the head produce
        # the standard [B, 4+nc, 8400] dense tensor we need for KD.
    ...
    # CPU vs GPU latency benchmark — always run, load model if it wasn't needed for splits
    if model is None:
    ...
    bench_results = benchmark_cpu_vs_gpu_latency(
        model, split_paths(SPLITS[0])["img_dir"])   # train se na edge ne salje

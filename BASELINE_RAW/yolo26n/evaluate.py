"""
Evaluate YOLO26n (COCO-pretrained) on train, val, and test splits of the
sub10k Open Images V7 dataset, in one go.

Layout:
    <DATASET_ROOT>/yolo26n/<split>/soft/<stem>.pt    pre-NMS dense (8400 anchors x 6 classes)
    <DATASET_ROOT>/yolo26n/<split>/labels/<stem>.txt post-NMS hard pseudo-labels (YOLO + conf)
    <DATASET_ROOT>/yolo26n/meta.json                 global metadata + per-split summary
...
def is_split_complete(paths: dict) -> bool:
    ...
    return image_stems.issubset(soft_stems) and len(image_stems) > 0
...
def run_inference_for_split(model: YOLO, split: str, paths: dict) -> dict:
    ...
    for img_path in img_paths:
        ...
        _soft_save(
            {"boxes_xywh": boxes_xywh, "class_probs": class_probs_ours,
             "letterbox": lb, "inference_ms": inf_ms},
            paths["soft_dir"] / f"{stem}.pt",
        )

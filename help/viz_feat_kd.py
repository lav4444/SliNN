"""
tmp_viz_feat_kd.py  --  TEMP vizualizacija feature tenzora koji se koristi za feature-KD.

Redovi   = 3 slike
Stupci   = original | TEACHER (yolo26l) | STUDENT 0.5M | STUDENT 1M | STUDENT 2M

Prikazuje ISTI tap (P3/P4/P5) i ISTI kanal kod svih modela. Smisleno je jer su studenti
tap-matched na egzaktne teacher tap-sirine (256/512/512) i KD je direktni MSE bez projekcije,
pa kanal c studenta odgovara kanalu c teachera.

Teacher featuri se citaju iz precomputanog cachea (yolo26l/train/feat/<stem>.pt) -- to je
doslovno tenzor koji trening koristi. Studenti se forwardaju iz KD_featlogit/checkpoints/best.pt.
...
SPLIT        = "train"                    # feat cache postoji samo za train
IMG_SIZE     = 640

IMAGE_STEMS  = None        # None = N_IMAGES slucajnih slika koje imaju feat cache; ili lista stemova
SEED         = 7
N_IMAGES     = 3

TAP          = 2           # 0 = P3 (256@80), 1 = P4 (512@40), 2 = P5 (512@20)
CHANNEL      = None        # None = automatski, kanal s najvecom prosjecnom aktivacijom kod teachera
MODE         = "channel"   # "channel" = jedan kanal | "magnitude" = L2 preko svih kanala
SHARED_SCALE = True        # ista skala boja unutar reda -> vide se razlike u magnitudi
CMAP         = "inferno"
...
# ===== Preprocess (identicno precompute_feats.py) =====
...
def load_student(exp_dir: Path):
    """Svaki student ima svoj model_arch_feat.py s istim imenom klase -> ucitavamo po putanji."""
...
def teacher_tap(stem):
    """-> [C, H, W] float32, iz precomputanog KD cachea."""
...
def main():
    ...
    # --- statistika = ono sto KD gubitak mjeri ---

"""
run_grow_experiment2.py — NETWORK GROWING: usporedba 4 kriterija dodavanja
jedinica na SchoolCNN-u (multi-label klasifikacija, 6 klasa).

Polaziste: pruned SchoolCNN (exp2 taylor.pt, ~5% params). Rastemo natrag preko
N_CYCLES SIMETRICNIH grow-FT ciklusa do GROW_TOTAL_FACTOR x pocetnih GFLOPs,
benefit/FLOP alokacija s per-layer capom (vidi criteria2.grow_to_budget).

Po tehnici (magnitude / gradient / taylor / hessian -> Net2Net / GradMax / Taylor /
Hessian-split):
  baseline (start) -> [ GROW chunk do kumulativnog GFLOPs cilja ->
                        FT do platoa (inner patience) ] x N -> finalni eval

Stop: N ciklusa | rani izlaz ako val mAP >= WANTED_MAP | outer stale.
Mjeri: params/GFLOPs, val/test macro mAP (+F1/acc), brzina (CPU/GPU), mrtve jedinice.
Izlaz: results.txt + summary.json + gflops_vs_map.png (4 trajektorije).

Pokretanje:  conda activate dipl && python run_grow_experiment2.py
"""
...
N_CYCLES = 3                         # simetrična 2 ciklusa growanja
GROW_TOTAL_FACTOR = 2.0              # cilj = 1.5x pocetni GFLOPs (+50%)
WANTED_MAP = 0.615                   # rani izlaz (≈ unpruned baseline val mAP 0.6165)
PATIENCE_FT = 4                      # inner: val mAP ne raste 2 epohe -> kraj FT-a
MAX_EP_PER_CYCLE = 30                # sigurnosna kapica epoha po ciklusu
STALE_CYCLES = 2                     # outer: 2 ciklusa bez globalnog napretka -> stop

CALIB_BATCHES = 16                   # za benefit (grad/taylor/hessian); magnitude=0
MAX_EVAL_IMAGES = 2000               # cap slika u eval (val 837 + test 1674 -> svi)
SEED = 42

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# (GFLOPs, test mAP) unpruned baseline SchoolCNN — referenca (iz exp2 summary.json)
...
def count_dead(model, calib, device, eps=1e-6):
    """Proxy mrtvih jedinica: ReLU izlazi (conv->bn->relu / fc->relu) koji nikad
    ne predu ~0 preko calib batcheva. SchoolCNN koristi ConvBNReLU + ReLU na fc."""
    acts = {}
    handles = []
    for nm, m in model.named_modules():
        if isinstance(m, nn.ReLU):
            # SchoolCNN dijeli jedan `model.act` za fc1 i fc2 -> disambiguiraj
            # kljuc sirinom izlaza (inace torch.maximum puca na razlicitim oblicima)
...
def ft_until_plateau(model, train_loader, val_loader, patience, max_ep, label):
    """BCE trening dok val mAP ne stagnira `patience` epoha (ili max_ep).
...
def run_technique(tech, f0_flops, start_map, loaders, calib_eval):
    ...
    for cycle in range(1, N_CYCLES + 1):
        ...
        t_cyc = time.time()
        # --- GROW do kumulativnog GFLOPs cilja (function-preserving) ---
    ...
    # finalni eval (val+test) + brzina + mrtve
...
def main():
    ...
    append_block("")
    calib_eval = C.make_calib_batches(8, DEVICE)        # za dead-count
    del base
    if DEVICE.type == "cuda":

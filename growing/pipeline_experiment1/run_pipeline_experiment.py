...
LR = TK.LR
WEIGHT_DECAY = TK.WEIGHT_DECAY
WARMUP_EPOCHS = TK.WARMUP_EPOCHS
CONSOLIDATION_PATIENCE = 8
...
def main():
    ...
    for epoch in range(1, TOTAL_EPOCHS + 1):
        ...
        if epoch <= GROW_EPOCHS:
            ...
            if cur < 0.98 * target:
                ...
                if plan:
                    grew = True
                    optimizer = new_optimizer(model, lr_at(gstep))
    ...
    if MODELS_DIR:
        ...
        torch.save(best_model, MODELS_DIR / f"{DISP}.pt")

"""
run_grow_experiment.py — NETWORK GROWING: usporedba 4 kriterija dodavanja filtera.

Polaziste: pruned StudentYOLO (taylor.pt, 50% params). Rastemo natrag preko 3
SIMETRICNA grow-FT ciklusa do 3x pocetnih GFLOPs (+200%), benefit/FLOP alokacija.

Po tehnici (magnitude / gradient / taylor / hessian):
  baseline (start) -> [ REZ... ne, GROW chunk do kumulativnog GFLOPs cilja ->
                        FT do platoa (inner patience) ] x 3 -> finalni eval

Stop: 3 ciklusa | rani izlaz ako val mAP >= WANTED_MAP | outer stale (2 ciklusa bez napretka).
...
#TECHNIQUES = ["gradient", "taylor", "hessian"]
...
N_CYCLES = 2                         # simetrična 2 ciklusa growanja
GROW_TOTAL_FACTOR = 1.2              # cilj = 1.1x pocetni GFLOPs (+10%)
WANTED_MAP = 0.17                    # rani izlaz
PATIENCE_FT = 5                      # inner: val mAP ne raste 5 epoha -> kraj FT-a
...
def main():
    ...
    append_block("=" * 78)
    append_block("NETWORK GROWING — usporedba 4 kriterija dodavanja filtera (StudentYOLO)")
    ...
    append_block(f"{N_CYCLES} simetrična ciklusa do {GROW_TOTAL_FACTOR:.0f}x pocetnih GFLOPs (+{(GROW_TOTAL_FACTOR-1)*100:.0f}%)")

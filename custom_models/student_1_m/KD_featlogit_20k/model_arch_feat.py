"""
model_arch_feat.py — tap-matched student (~1M) za KD_featlogit eksperiment.

Identičan StudentYOLO (custom_models/student_1_m/model_arch.py) u backboneu/necku/API-ju,
ALI izlaz neka je projiciran na EGZAKTNE tap-širine yolo26l teachera (P3=256, P4=512, P5=512)
pa je feature-KD egzaktni MSE (kao morphology pipeline) — bez PCA, bez projekcije na usporedbi.

Tap (= ulaz u glavu) širine: [256, 512, 512] @ [80, 40, 20]. Glava odmah reducira tap
nazad na 64 (jeftino) -> params ostaju ~1M.
...
class StudentYOLOFeat(nn.Module):
    ...
    def __init__(self, num_classes: int = NUM_CLASSES, input_size: int = INPUT_SIZE,
        ...
        # ---- Backbone (isti kao 1M student) ----
        self.stem = ConvBNAct(3, 24, k=3, s=2)
        self.dark2 = nn.Sequential(
            ConvBNAct(24, 48, k=3, s=2),
            ConvBNAct(48, 48, k=3, s=1),
        )
        self.dark3 = nn.Sequential(    # -1 blok vs 1M student (tap-matching košta ~167K, vraćamo ~120K ovdje + dark4)
            ConvBNAct(48, 64, k=3, s=2),
            ConvBNAct(64, 64, k=3, s=1),
            ConvBNAct(64, 64, k=3, s=1),
        )
        self.dark4 = nn.Sequential(    # -1 blok vs 1M student
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.zeros_(m.bias)
        # bias final 1x1 cls -> initial sigmoid prob ~0.01 (standardni YOLO trik)
        prior = 0.01

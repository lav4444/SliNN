...
from model_arch import StudentYOLO  # noqa: F401  (referenca; ne koristi se direktno)
...
TEACHER_FEAT_DIR = DATASET_ROOT / "yolo26l" / "train" / "feat"
...
class KDTrainDataset(Dataset):
    ...
    def __init__(self, images_dir: Path, soft_dir: Path, feat_dir: Path):
        all_imgs = sorted(p for p in images_dir.iterdir()
                          if p.suffix.lower() in (".jpg", ".jpeg", ".png"))
        self.items = [p for p in all_imgs
                      if (soft_dir / f"{p.stem}.pt").exists() and (feat_dir / f"{p.stem}.pt").exists()]
        self.soft_dir = soft_dir
        self.feat_dir = feat_dir
        if not self.items:
            raise RuntimeError(f"No (image, soft, feat) triples in {images_dir}")
        print(f"[KDTrainDataset] {len(self.items)}/{len(all_imgs)} images have soft+feat targets")
    ...
    def __getitem__(self, idx):
        ...
        feat = torch.load(self.feat_dir / f"{p.stem}.pt", weights_only=True)["feat"]
        return {
            "image": img_t,
            "teacher_boxes": soft["boxes_xywh"].float(),
            "teacher_probs": soft["class_probs"].float(),
            "teacher_feat": [f.float() for f in feat],
        }
...
def kd_loss(student_feat, raw_student, decode_fn, teacher_feat, teacher_boxes, teacher_probs):
    boxes_xywh, class_probs = decode_fn(raw_student)
    s_box = xywh_to_xyxy(boxes_xywh)
    t_box = xywh_to_xyxy(teacher_boxes)
    t_conf = teacher_probs.amax(dim=-1)
...
def main():
    ...
    train_ds = KDTrainDataset(TRAIN_IMG_DIR, TEACHER_SOFT_DIR, TEACHER_FEAT_DIR)

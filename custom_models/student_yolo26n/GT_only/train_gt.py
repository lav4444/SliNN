"""
Train StudentYOLO (yolo26n arhitektura, ~2.5M) on GROUND-TRUTH ONLY — kontrolni
baseline za KD studiju. Trening SAMO na part1 (5860 slika), kao i svi ostali
GT_only runovi, pa je usporediv s KD_featlogit part1 rezultatom.

RAZLIKA OD OSTALIH GT_only SKRIPTI
----------------------------------
yolo26n postavlja BOX_OUTPUT_FORMAT = "decoded": njegovi box kanali su vec
xywh u 640x640 pikselima (Detect glava radi vlastiti dist2bbox), dok custom
studenti daju anchor-relativni raw. Zato se u gt_supervision_loss dekodiranje
preskace kad je format "decoded". Klasni kanali su LOGITI u oba slucaja
(model_arch.py obrce sigmoid), pa focal loss ostaje nepromijenjen.

Purpose:
  Isolates the *supervision signal*. Everything (architecture, optimizer, schedule,
  augmentation=none, epochs, early-stop, eval protocol) is IDENTICAL to the pure_KD
  baseline; the ONLY difference is the target source:

      pure_KD :  cls = teacher soft-probs,  box = teacher boxes (conf-weighted)
      GT_only :  cls = GT one-hot,          box = GT boxes (best-IoU anchor)

  Loss: sigmoid focal on cls + (v2) 1-GIoU on decoded box, weights w_cls=1.5, w_box=1.0,
  focal alpha=0.25 gamma=2.0. No teacher, no soft labels, no feature distillation. GT
  anchor assignment reuses the exact machinery from KD_GT_FGD (load_gt_yolo_letterboxed /
  assign_gt_to_anchors). NOTE: v1 used SmoothL1 on encoded coords; v2 switches the box
  loss to GIoU to align with the IoU-based mAP metric (see gt_supervision_loss).

  Same train set (5860 imgs) and same val set (837 imgs) as the KD runs -> the
  resulting eval_result.txt is a drop-in extra column for the KD-vs-GT comparison.
...
# ===== Config (identical protocol to pure_KD) =====
...
LOSS_W_CLS = 1.5
LOSS_W_BOX = 2.0     # v2: raised from 1.0 — GIoU is in [0,2] (different scale than SmoothL1 sum)
FOCAL_ALPHA = 0.25
FOCAL_GAMMA = 2.0

# Weight EMA (Ultralytics-style, warmup ramp). EMA weights validate + get saved as best.pt.
EMA_DECAY = 0.9999
EMA_TAU = 2000
...
# ===== GT labels (ported verbatim from KD_GT_FGD) =====
...
def assign_gt_to_anchors(gt_boxes_xyxy: torch.Tensor, gt_labels: torch.Tensor,
    ...
    If multiple GTs map to the same anchor, the higher-IoU one wins (greedy sorted).
...
# ===== GT loss (same FORM as pure_KD: focal cls + SmoothL1 box) =====
...
def sigmoid_focal_loss_sum(logits, targets, alpha=FOCAL_ALPHA, gamma=FOCAL_GAMMA):
    """Sigmoid focal loss (Lin et al.), soft/one-hot targets. Returns SUM; caller / num_pos."""
...
def _decode_box_xywh(raw_box, anchor_xy, anchor_stride):
    """Mirror of StudentYOLO.decode for boxes: raw [.., 4] -> xywh px. Expects fp32 inputs."""
...
def _giou(a, b):
    """Generalized IoU on xyxy boxes (Rezatofighi et al.). a, b: [..., 4]. Returns [...]."""
...
def gt_supervision_loss(raw_student, cls_targets, box_targets, pos_mask, anchor_xy, anchor_stride,
                        box_format="raw"):
    """Direct GT loss: focal on cls (positives -> class one-hot), 1-GIoU on box (positives only).

    Box loss (v2): GIoU on DECODED xyxy in pixel space, computed in fp32 (decoded areas
    reach ~640^2 which overflows fp16 under AMP). Replaces the v1 SmoothL1-on-encoded-coords
    to better align the box objective with the IoU-based mAP metric (scale-invariant, couples
    all 4 coords). cls loss and everything else are unchanged.

    raw_student:  [B, 4+nc, 8400]
    cls_targets:  [B, 8400, nc]
    box_targets:  [B, 8400, 4]   (cx,cy,w,h px)
    pos_mask:     [B, 8400]
    """
    ...
    if pos_mask.any():
        # fp32 throughout: decoded px boxes overflow fp16 (area up to ~640^2).
        if box_format == "decoded":
            # yolo26n: box kanali su VEC xywh u px -> nema dekodiranja.
...
class GTTrainDataset(Dataset):
    """Returns image + precomputed GT anchor assignment (no teacher, no soft labels)."""
...
class ValDataset(Dataset):
    """Validation: image + GT (xyxy in original px for mAP) + GT anchor assignment (for val-loss diag)."""
    ...
    def __getitem__(self, idx):
        ...
        # GT in ORIGINAL px for mAP
        gt_boxes, gt_labels = self._load_gt(self.labels_dir / f"{p.stem}.txt", orig_w, orig_h)
        # GT anchor assignment (letterbox space) for the val-loss diagnostic
...
def postprocess_for_eval(decoded_boxes, decoded_probs, conf_thresh, iou_thresh, max_det):
    """boxes [8400,4] xywh in letterbox space; probs [8400, nc]. Returns NMSed (xyxy, scores, labels)."""
...
def validate(model, val_loader, device, anchor_xy, anchor_stride):
    """Single pass over val: computes (a) val GT loss, (b) val mAP vs GT."""
    model.eval()
...
# ===== Weight EMA =====
class ModelEMA:
    """Exponential Moving Average of model weights (Ultralytics-style, with warmup ramp).

    After each optimizer step:  ema = d*ema + (1-d)*model, where d ramps from ~0 to
    EMA_DECAY over ~EMA_TAU steps so early noisy weights don't dominate. Params AND
    float buffers (BN running_mean/var) are averaged; integer buffers are left as-is.
    The EMA copy generalizes better -> used for validation and saved as best.pt.
    """
...
def main():
    ...
    # "raw" za custom studente; yolo26n postavlja "decoded".
    box_format = getattr(model, "BOX_OUTPUT_FORMAT", "raw")
    ...
    # Sanity: average positives per image (too few -> assignment starved).
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        ckpt = {
            ...
            "ema": ema.ema.state_dict(),        # EMA weights (for deployment/eval)
        ...
        if is_val_epoch:
            ...
            if improved:
                ...
                # best.pt ships the EMA weights under "model" so evaluate_student.py (which
                # loads ckpt["model"]) evaluates exactly what the val mAP above measured.
                best_ckpt = dict(ckpt)

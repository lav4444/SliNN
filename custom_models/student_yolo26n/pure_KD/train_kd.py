...
Validation every VAL_EVERY_N_EPOCHS epochs (1, 4, 7, ...): val KD loss (vs teacher)
+ val mAP (vs hard GT). Validation is the slow step (~2.5x training step) so we
skip 2 out of every 3 epochs to save time.
Early stops on val mAP@50:95 with patience counted in epochs.
Reduces LR by LR_REDUCE_FACTOR on a shorter patience (LR_REDUCE_PATIENCE) — gives
the model a chance to escape a plateau before fully giving up.
...
VAL_EVERY_N_EPOCHS = 3    # validate on epochs 1, 4, 7, 10, ... to save time
EARLY_STOP_PATIENCE = 9   # stop if val mAP@50:95 doesn't improve for this many epochs (= 3 validation checks given VAL_EVERY_N_EPOCHS=3)
LR_REDUCE_PATIENCE = 6    # reduce LR if val mAP@50:95 doesn't improve for this many epochs (= 2 validation checks); fires before early-stop
LR_REDUCE_FACTOR = 0.6    # multiplier applied to LR each time the reduction triggers (can fire again on later plateaus after a fresh best)
...
def kd_loss(raw_student, teacher_boxes, teacher_probs, anchor_xy, anchor_stride, box_format="raw"):
    """
    raw_student:    [B, 4+nc, 8400]
        If box_format == "raw":     student box channels are anchor-offset + log-size
        If box_format == "decoded": student box channels are xywh in 640x640 px (yolo26n)
        In both cases student cls channels are LOGITS (pre-sigmoid).
    teacher_boxes:  [B, 8400, 4]   (cx,cy,w,h in 640x640 letterbox px — always decoded)
    teacher_probs:  [B, 8400, nc]  (sigmoid'd, our 6 classes)
    anchor_xy:      [8400, 2]      (anchor centers in px)
    anchor_stride:  [8400]
    """
    student_box_part = raw_student[:, :4, :].permute(0, 2, 1)     # [B, 8400, 4]
    ...
    # (any anchor where teacher is confident about some class). Identical across
    # both box_formats — cls channels are LOGITS in either case.
    ...
    # Box loss: SmoothL1, weighted by max teacher prob.
    if box_format == "raw":
        # Student box is anchor-offset + log-size. Encode teacher to same space and compare.
        target_raw = encode_box_to_raw(teacher_boxes, anchor_xy, anchor_stride)
        box_diff = F.smooth_l1_loss(student_box_part, target_raw, reduction="none").sum(-1)
    elif box_format == "decoded":
        # Student box is xywh px (yolo26n style). Compare in stride-normalized px so the
        # per-anchor loss magnitude is comparable to raw-space loss across P3/P4/P5.
        stride = anchor_stride.view(1, -1, 1)        # [1, 8400, 1] broadcast over batch & xywh
        box_diff = F.smooth_l1_loss(
            student_box_part / stride,
            teacher_boxes / stride,
            reduction="none",
        ).sum(-1)
    else:
...
def validate(model, val_loader, device, anchor_xy, anchor_stride):
    ...
    for batch in val_loader:
        ...
        loss, lcls, lbox = kd_loss(raw, tboxes, tprobs, anchor_xy, anchor_stride, box_format=box_format)
        sum_loss += float(loss)
        sum_cls += float(lcls)
        sum_box += float(lbox)
        n_batches += 1
...
def make_training_plots(history: dict, out_path: Path, best_epoch: int | None):
    ...
    ax.plot(val_epochs, history["val_kd_loss"], label="val (KD)", color="tab:orange", linewidth=1.6, marker="o", markersize=4)
...
def main():
    ...
    history = {
        ...
        # per-validation (sparse: only on val epochs)
        "val_epoch": [],
        "val_kd_loss": [], "val_cls": [], "val_box": [],
        "val_map": [], "val_map_50": [], "val_map_75": [],
    }

    # Read box output format from the model (see model_arch.py for definition).
    # Defaults to "raw" for our custom students; yolo26n student sets "decoded".
    box_format = getattr(model, "BOX_OUTPUT_FORMAT", "raw")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch in train_loader:
            ...
            with torch.amp.autocast("cuda", enabled=device.type == "cuda"):
                raw = model(imgs)
                loss, loss_cls, loss_box = kd_loss(raw, tboxes, tprobs, anchor_xy, anchor_stride, box_format=box_format)
        ...
        if is_val_epoch:
            ...
            if improved:
                ...
                # LR reduction: fires once per non-improvement streak, before early-stop kicks in.
                # Both the active LR and scheduler.base_lrs are reduced — otherwise the next
                # scheduler.step() would overwrite our reduction (it multiplies the cosine
                # lambda against base_lr each step).

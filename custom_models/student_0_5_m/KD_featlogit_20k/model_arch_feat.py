"""
model_arch_feat.py — tap-matched 0.5M student za KD_featlogit eksperiment.

Identičan StudentYOLO (custom_models/student_0_5_m/KD_first.py) u backboneu/necku/API-ju,
ALI izlaz neka (ulaz u glavu) projiciran je na EGZAKTNE tap-širine yolo26l teachera
(P3=256, P4=512, P5=512) pa je feature-KD egzaktni MSE (kao morphology pipeline) — bez PCA.

PAŽNJA: tap-trošak (proj + glave na teacher-širine) je ~FIKSAN (~120K) jer ovisi o TEACHERU,
ne o studentu. Na 0.47M bazi to je +~26% (za razliku od +4.6% na 1M, +0.9% na 2M) — vidi
komentar uz param-budžet u raspravi. Backbone JE trimovan (-1 blok u dark4 i dark5, ~-120K)
da se kompenzira tap-trošak i zadrži ~0.47M (≈ referenca); dark3/P3 tap ostaje pun.

forward(x)                     -> raw [B, 4+nc, 8400]
...
class StudentYOLOFeat(nn.Module):
    ...
    def __init__(self, num_classes: int = NUM_CLASSES, input_size: int = INPUT_SIZE,
        ...
        # ---- Backbone (isti kao 0.5M student) ----
        ...
        self.dark4 = nn.Sequential(    # -1 blok vs 0.5M student (kompenzira tap-trošak)
            ConvBNAct(48, 64, k=3, s=2),
            ConvBNAct(64, 64, k=3, s=1),
        )
        self.dark5 = nn.Sequential(    # -1 blok vs 0.5M student
        ...
        # ---- Tap projekcije: neck (48) -> EGZAKTNE teacher tap-širine (= feature-KD tap) ----
        ...
        # ---- Heads: tap-width -> 1x1 redukcija na neck_ch -> 3x3 -> 1x1 no ----
    ...
    def _make_head(self, in_ch: int, mid_ch: int) -> nn.Sequential:
        return nn.Sequential(
            ConvBNAct(in_ch, mid_ch, k=1, p=0),    # jeftina redukcija tap -> neck_ch

"""
Evaluate trained StudentYOLO on train, val, and test splits.

Mirrors baseline_models/yolo26l/eval_result.txt format so the two reports can
be put side-by-side for a clean teacher-vs-student comparison.
...
# TRAIN eval is slow (~9 min: torchmetrics mAP over 5860 imgs at conf=0.001). VAL+TEST are
# what the comparison table needs; add "train" back only when you want the overfit diagnostic.
SPLITS = ("val", "test")

# Cap images per split for eval speed (torchmetrics mAP is ~linear in image count).
# Seeded random subset so it's reproducible. VAL (837) stays full; TEST (1674) is subsampled.
# NOTE: capped-split mAP is on a subset, so it's not 1:1 with the KD models' full-split eval;
# keep cross-model comparisons on VAL (uncapped at <=1000).
EVAL_MAX_IMAGES_PER_SPLIT = 1000
EVAL_SUBSET_SEED = 42
...
def eval_split(model, split: str, device: torch.device) -> dict:
    ...
    if len(img_paths) > EVAL_MAX_IMAGES_PER_SPLIT:
        img_paths = sorted(random.Random(EVAL_SUBSET_SEED).sample(img_paths, EVAL_MAX_IMAGES_PER_SPLIT))
        print(f"[{split}] capped {len(list_images(img_dir))} -> {EVAL_MAX_IMAGES_PER_SPLIT} images (seed={EVAL_SUBSET_SEED})")
...
def write_eval_result(per_split: dict, ckpt_info: dict, ckpt_path: Path,
    ...
    lines = [
        ...
        f"(val_gt_loss={ckpt_info.get('val_gt_loss', float('nan')):.4f}, "
...
def benchmark_cpu_vs_gpu_latency(model: StudentYOLO, train_img_dir: Path) -> dict:
    ...
    for img_path in sampled:
        ...
        inputs.append(img_t.unsqueeze(0))   # [1, 3, 640, 640]
...
def main():
    ...
    print(f"Loaded checkpoint from epoch {ckpt.get('epoch', '?')} "
          f"(val_gt_loss={ckpt.get('val_gt_loss', float('nan')):.4f}, "

"""
Evaluate trained StudentYOLOFeat (KD_featlogit) on train, val, and test splits.

Mirrors baseline_models/yolo26l/eval_result.txt and pure_KD/eval_result.txt format
so teacher / pure_KD / featlogit reports can be put side-by-side.

Pokreni NAKON treninga (čita checkpoints/best.pt):
    conda activate dipl
...
def eval_split(model, split: str, device: torch.device) -> dict:
    ...
    for img_path in img_paths:
        ...
        raw = model(img_t)                          # return_feats=False -> raw [1, no, 8400]
...
def write_eval_result(per_split: dict, ckpt_info: dict, ckpt_path: Path,
    ...
    lines = [
        f"Model: StudentYOLOFeat ({n_params:,} params)  [KD featlogit: feature + dense_cls + box_giou, no GT]",
        f"Checkpoint: {ckpt_path}",
        f"Trained epoch: {ckpt_info.get('epoch', '?')} "
        f"(val_resp_loss={ckpt_info.get('val_kd_loss', float('nan')):.4f}, "
...
def benchmark_cpu_vs_gpu_latency(model: StudentYOLOFeat, train_img_dir: Path) -> dict:
    rng = random.Random(BENCHMARK_SEED)
    all_imgs = list_images(train_img_dir)
    if len(all_imgs) < BENCHMARK_N_IMAGES:
        raise RuntimeError(f"Need {BENCHMARK_N_IMAGES} images, only {len(all_imgs)} found in {train_img_dir}")
    ...
    for device_name in ("cpu", "cuda"):
        ...
        results[device_name] = {"all_ms": times_ms,
...
def format_benchmark_results(results: dict) -> str:
    ...
    cpu_r = results.get("cpu"); gpu_r = results.get("cuda")
...
def main():
    ...
    model = StudentYOLOFeat(num_classes=NUM_CLASSES, input_size=IMG_SIZE).to(device).eval()

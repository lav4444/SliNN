"""
model_arch_feat.py — tap-matched 2M student za KD_featlogit eksperiment.

Identičan StudentYOLO (custom_models/student_2_m/model_arch.py: SPPF + PAN neck) u
backboneu/necku/API-ju, ALI izlaz neka (ulaz u glavu) projiciran je na EGZAKTNE tap-širine
yolo26l teachera (P3=256, P4=512, P5=512) pa je feature-KD egzaktni MSE (kao morphology
pipeline) — bez PCA, bez projekcije na usporedbi.

Tap = ulazi u glave (bottom-up izlazi PAN-a) projicirani na [256, 512, 512] @ [80, 40, 20].
Glava odmah reducira tap nazad na 64 (jeftino) -> params ostaju ~2M.
Jedan ConvBNAct(128,128) izbačen iz dark4 da kompenzira ~167K tap-troška (≈ reference 2.07M).

forward(x)                     -> raw [B, 4+nc, 8400]            (drop-in kao StudentYOLO)
forward(x, return_feats=True)  -> (raw, [t3, t4, t5])           (KD: tap featuri, redoslijed P3,P4,P5)
decode(raw)                    -> (boxes_xywh [B,8400,4], class_probs [B,8400,nc])
"""
...
class SPPF(nn.Module):
    """Spatial Pyramid Pooling - Fast (YOLOv5+); 3× chained 5x5 maxpool."""
...
TEACHER_TAP_CH = (256, 512, 512)   # yolo26l Detect-head ulazi (P3,P4,P5) — egzaktne tap-širine
...
class StudentYOLOFeat(nn.Module):
    ...
    def __init__(self, num_classes: int = NUM_CLASSES, input_size: int = INPUT_SIZE,
        ...
        # ---- Backbone (isti kao 2M student; -1 blok u dark4 radi tap-troška) ----
        ...
        self.dark4 = nn.Sequential(    # -1 blok vs 2M student (kompenzira ~167K tap-troška)
        ...
        # ---- Tap projekcije: neck (64) -> EGZAKTNE teacher tap-širine (= feature-KD tap) ----
        ...
        # ---- Heads: tap-width -> 1x1 redukcija na 64 -> 3x3 -> 1x1 no ----
    ...
    def _make_head(self, in_ch: int, mid_ch: int) -> nn.Sequential:
        return nn.Sequential(
            ConvBNAct(in_ch, mid_ch, k=1, p=0),    # jeftina redukcija tap -> 64
    ...
    def forward(self, x: torch.Tensor, return_feats: bool = False):
        ...
        # Tap featuri (== teacher širine) — redoslijed P3, P4, P5
        t3 = self.proj_p3(bu_p3)
        t4 = self.proj_p4(bu_p4)
        t5 = self.proj_p5(bu_p5)

        # Heads
        o3 = self.head_p3(t3)   # [B, no, 80, 80]
        o4 = self.head_p4(t4)   # [B, no, 40, 40]
        o5 = self.head_p5(t5)   # [B, no, 20, 20]

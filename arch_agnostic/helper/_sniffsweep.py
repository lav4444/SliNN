"""_sniffsweep.py — testira SAMO agnostic_sniffer (zaobilazi kaskadu) na svakom datasetu.
Cist test generalizirajuceg fallbacka: sto nadje kao ULAZE + kako pogadja OZNAKE, bez specificnih recognizera.
Rezultat -> REPORTS/sniff_report.txt (+ ispis)."""
import os
import sys
...
# 6 distinktnih dataset-putanja (yolo/frcnn/schoolcnn dijele sub10k)
PATHS = [
...
for name, path in PATHS:
    r = DS.agnostic_sniffer(path, cap=500000)                 # visok cap -> stvarni brojevi
    if r is None:

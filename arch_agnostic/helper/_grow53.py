"""_grow53.py — smoke za Fazu 5.3 (grow: GradMax iz gavg + churn-cooldown) na voc_deeplabv3.
  A) morph_loop (prune+grow+cooldown+reinvest) -> trajektorija; provjeri: net GFLOPs pada, forward OK,
     i (ako grow opali) nijedan NARASTAO sloj nije rezan unutar cooldown-prozora.
  B) IZRAVNI grow unit-test (GROW_DOM privremeno spusten, velik pool): dokaz da _grow_decide iz NASEG gavg
     materijalizira FUNCTION-PRESERVING rast (params rastu, forward OK, izlaz ~nepromijenjen).
-> REPORTS/grow53.txt
"""
import copy
import os
...
# ================= A) morph_loop (prune + grow + cooldown) ================= #
...
# cooldown provjera: nijedan narastao sloj rezan unutar prozora? (pruned_at vs grown_at, isti sloj, |Δ|<=cooldown)
churn = []
...
# ================= B) izravni grow unit-test (dokaz function-preserving mehanike) ================= #
# _grow_decide na PUNOM modelu zna vratiti None (tp-fragilnost na top kandidatima -> konzervativno odustane;
# u petlji A radi jer je model prorezan). Zato ovdje IZRAVNO prebrojimo koliko prunable slojeva prima
# function-preserving +1 rast preko _try_grow_layer (izolira widening-mehaniku od grow-ekonomije).
...
for nm in prunable:
    grown = C._try_grow_layer(student, ad, dev, nm, 1)          # +1 kanal, clone-init (function-preserving)
    if grown is None:
        continue
    with torch.no_grad():

...
from classify import weighted_leaves as _WL_AG              # lokalni (dim 2/3/4) -> vidi Conv1d/1D lanac

TMP_ROOT = os.path.join(_AA, "tmp")                          # cache dom (regenerabilno; gitignore)
...
def install_sizing_shims():
    ...
    pozivatelji (bare-name kroz module-globale):
      ...
      A.weighted_leaves  -> dim (2,3,4) (vidi Conv1d/1D lanac); vide `_apply_prune_plan`/`coupled_unit_cost`/…
...
def materialize_train_batches(path, adapter, device, split_plan, batch_size=8, n_batches=8, seed=0):
    """FIKSNI train-batchevi kao CPU tenzori (reuse kroz epohe). Vrati (batches, source).
    batches = lista <=n_batches lista po <=batch_size uzoraka. Deterministicno (seed); fallback-random se
    generira JEDNOM i drzi (frozen) -> teacher-cache alignment vrijedi i bez readera (token/parquet)."""
    import random
    ...
    split = None if (tr in (None, "AUTO")) else tr           # 'AUTO' -> pool cijelog stabla
...
def morph_loop(student, teacher, adapter, device, ctx, path, model_name,
    ...
    for step in range(1, max_steps + 1):
        if (g0 - traj[-1]["gflops"]) >= target_frac * g0:
            break
        morph_idx += 1
        grow_protected = {l for l, k in grown_at.items() if morph_idx - k <= cooldown}   # svjež narastao -> ne rezi
        prune_protected = {l for l, k in pruned_at.items() if morph_idx - k <= cooldown}  # svjež rezan -> ne rastri
        ...
        # ---- PRUNE (cooldown-svjestan, forward-safe, ban+re-select) ----
        ...
        for _try in range(5):
            ...
            if not plan and grow_protected and not cd_override:      # cooldown blokira SVE -> rez nužan: SOFT override
        ...
        if metric_fn is not None:
            if cur_metric >= metric_floor:
                ...
                if below >= 3:                                        # metrika probila floor 3 koraka zaredom -> stani
                    break
        if n_rem == 0 and not grown_info and not grow_protected:      # ništa rezano ni naraslo, bez cooldowna -> iscrpljeno
            break
...
def full_cycle(student, teacher, adapter, device, ctx, path, model_name,
    ...
    if not kw.get("metric_fn"):
        import metric as _M
        gate_inputs = [s for b in batches for s in b][:64]       # sirovi uzorci (bez oznaka); dovoljno za stabilan signal
        kind = ctx["out_kind"]
        kw["metric_fn"] = lambda mdl: _M.teacher_agreement(mdl, teacher, adapter, gate_inputs, device, kind=kind)["agreement"]
        kw.setdefault("metric_tol", 0.97)                        # zadrži ≥97% slaganja s učiteljem

"""
run_experiment2.py — usporedba 4 kriterija vaznosti za STRUCTURED pruning na
skolskom CNN-u (multi-label klasifikacija, 6 klasa).

Razlika vs exp1: arhitektura ima i CONV FILTERE i DENSE NEURONE -> pruning rezhe
oboje (vidi pruning_lib2). Inace isti protokol.

Po metodi (magnitude|gradient|taylor|hessian):
  1) vaznost + OVERHEAD (vrijeme, GPU mem, /slika)
  2) rezanje do istog budzeta params (zadrzi KEEP_PARAM_FRAC = 70% -> izbaci 30%)
  3) eval BEZ FT (BN rekalibriran) na val/test
  4) fine-tune (max 10 epoha, isti LR, early-stop patience 3 na val_loss)
  5) eval NAKON FT + benchmark brzine (CPU/GPU)

Baseline CNN se trenira od nule ako checkpoint ne postoji.
Rezultat: results.txt (+ summary.json + pruned_models/<metoda>.pt). Bez trening logova.

Pokretanje (rucno, env dipl):
    conda activate dipl
    cd /home/tomi/code/dipl/pruning/critereum_experiment2
...
KEEP_PARAM_FRAC = 0.05      # zadrzi 5% -> izbaci 95% (vrlo agresivno; duboko grize zive jedinice + convove)
MIN_KEEP = 2
ALLOC = "global"

CALIB_BATCHES = 32          # batcheva za vaznost i BN rekalibraciju
DO_RECALIB_EVAL = True      # no-FT eval (BN rekalibriran)

EVAL_SPLITS = ("val", "test")
MAX_EVAL_IMAGES = 2000      # cap slika po splitu u evaluaciji (val 837 i test 1674 -> svi)

FT_MAX_EPOCHS = 10          # fine-tune max 10 epoha (patience kao baseline)
FT_LR = TB.LR               # isti kao (vec snizen) baseline LR ~3.33e-4 (original/3)
SAVE_MODELS = True
...
def main():
    ...
    # ---- baseline checkpoint (treniraj ako fali) ----
    if not BASE_CKPT.exists():
        TB.train_baseline_and_save(DEVICE, BASE_CKPT)

    # ---- dijeljeni loaderi ----
    # eval (capped, bez shuffle) - isti za sve metode
    ...
    # kalibracijski batchevi (vaznost + BN rekalib) - isti za sve metode
    calib_batches = []

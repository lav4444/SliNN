"""
pruning_lib2.py — STRUCTURED pruner za SchoolCNN: reze i CONV FILTERE i DENSE NEURONE.

Sekvencijalna arhitektura (nema concat/skip), pa je dependency graf jednostavan:
  conv1 -> conv2 -> ... -> conv5 -> [flatten] -> fc1 -> fc2 -> fc3(izlaz, ne rezati)

Jedina netrivijalna veza je FLATTEN: kad orezemo izlazni kanal c zadnjeg conv-a,
iz ulaza prvog dense sloja nestaje cijeli BLOK od (H*W) znacajki (flatten je
channel-major: kanal c -> indeksi [c*HW : (c+1)*HW]). To rjesava 'block' u izvoru.

Kriteriji (po izlaznoj jedinici o, zbroj preko ostatka tezina; radi i za conv i
za linear jer flatten(1) izravna sve osim izlazne dimenzije):
  magnitude :  sum |W_o|
  gradient  :  sum |g_o|
  taylor    :  ( sum g_o*W_o )^2
  hessian   :  0.5 * sum (g2_o * W_o^2)   (OBD; empirijski Fisher ~ dijag. Hessiana)
"""
...
def build_plan(model):
    ...
    add_linear("fc1", "fc1", [("conv5", model.flatten_hw)])   # FLATTEN: block=H*W
    add_linear("fc2", "fc2", [("fc1", 1)])
    add_linear("fc3", "fc3", [("fc2", 1)], prune_out=False)   # izlaz (6) ne rezati
...
# --------------------------------------------------------------------------- #
# Brojanje parametara iz stanja (counts: dict name->broj izlaznih jedinica)
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# Izbor kanala/neurona (globalni rang, per-layer L2 norm, do budzeta params)
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# FLOPs (conv + linear) i BN rekalibracija
# --------------------------------------------------------------------------- #
@torch.no_grad()

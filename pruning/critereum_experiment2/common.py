"""
common.py — dataset (multi-label), metrike i benchmark brzine za exp2.

Multi-label meta se izvodi iz YOLO label datoteka: za svaku sliku, klasa je
"prisutna" ako se pojavljuje barem jednom u .txt datoteci. Slika bez labela ->
sve nule (pozadina).
"""
...
# --------------------------------------------------------------------------- #
# Evaluacija (loss + macro mAP + macro F1 + accuracy)
# --------------------------------------------------------------------------- #
@torch.no_grad()
def evaluate(model, loader, device, criterion=None):
    ...
    return {
        ...
        "map": float(per_class_ap.mean()),          # macro mAP
        "f1": float(f1.compute()),                  # macro F1@0.5
        "acc": float(acc.compute()),                # macro (per-label) accuracy@0.5
...
# --------------------------------------------------------------------------- #
# Benchmark brzine (CPU vs GPU): 2 warmup (odbaceni) + ostatak mjeren
# --------------------------------------------------------------------------- #

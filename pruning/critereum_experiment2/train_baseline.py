"""
train_baseline.py — trening SchoolCNN-a (multi-label) + dijeljena trening petlja.

Baseline: full trening, fiksni LR, early-stop kad se val_loss ne poboljsa
PATIENCE epoha. Ista petlja sluzi i za fine-tune (samo s manjim max_epochs).

Standalone:
    conda activate dipl
    python train_baseline.py        # trenira i sprema checkpoints/best.pt
"""
...
# Univerzalni hiperparametri (isti za baseline i fine-tune)
LR = 1e-3 / 3.0          # snizen 3x (originalno 1e-3 -> ~3.33e-4); koristi i baseline i FT
BATCH_SIZE = 32
NUM_WORKERS = 4
PATIENCE = 3              # early-stop: val_loss se ne poboljsa PATIENCE epoha
BASELINE_MAX_EPOCHS = 100  # sigurnosna kapica; efektivno "do early stopa"
SEED = 42


def train_loop(model, device, train_loader, val_loader, lr, max_epochs, patience, label=""):
    """Trenira model; early-stop na val_loss (patience epoha). Vraca (best_state, stats)."""
...
def train_baseline_and_save(device, ckpt_path=CKPT_PATH):
    """Istrenira baseline od nule i spremi best (po val_loss)."""

# -*- coding: utf-8 -*-
"""FAKTORIJELNA ABLACIJA kriterija vaznosti i normalizacije u SliNN prune petlji.

Prvi run (slinn_importance_ablation) je pokazao taylor+mean > grad+none (+0.019 mAP), ali:
  (a) ne zna RAZLOZITI dolazi li dobitak od kriterija, od normalizacije ili od njihove sprege,
  (b) trajektorija se dvaput krizala (koraci 6-7), a bio je JEDAN seed.

Zato 2x2 faktorijel {grad,taylor} x {none,mean}, svaki na 3 SEEDA. Seed mijenja KOJI podskup
train podataka ulazi (materialize_train_batches) — pri fiksnim batchevima je petlja gotovo
deterministicna, pa bi ponavljanje istog seeda mjerilo samo cuDNN sum, ne pravu varijancu.

Kontrolirano: isti pocetni model (deepcopy), isti teacher, ISTI batchevi unutar seeda za sve 4
celije, fiksno STEPS koraka bez early-stopa, GROW iskljucen (grow signal je isti u svim celijama).
"""
...
CELLS = [                                                   # (ime, kriterij, normalizacija)
    ('grad_none',   'grad',   'none'),                      # postojece
    ('grad_mean',   'grad',   'mean'),                      # glavni efekt NORMALIZACIJE
    ('taylor_none', 'taylor', 'none'),                      # glavni efekt KRITERIJA
    ('taylor_mean', 'taylor', 'mean'),                      # interakcija
]
...
def main():
    ...
    S = W._setup(cfg, dev)                                  # model/teacher/metrika: JEDNOM za sve
    ...
    for seed in SEEDS:
        # ISTI batchevi za sve 4 celije unutar seeda -> razlika je iskljucivo u rangiranju

"""
run_norm_experiment.py — ablacija NORMALIZACIJE važnosti za structured pruning.

Isti protokol kao exp3 (production detektor fasterrcnn_mobilenet_v3_large_320_fpn,
Torch-Pruning, param-target 70% total params), ali:
    - KRITERIJ je FIKSAN = Taylor (g*w)
    - varira se SAMO `normalizer` (per-grupa skaliranje važnosti prije globalnog
      rangiranja) -> izolira utjecaj normalizacije na tocnost pri istom budzetu.

Normalizeri (tp `_normalize`):
    none           -> bez normalizacije (kontrola; izlozen cross-layer scale problem)
    mean           -> x / mean(x)            (dosadasnji default)
    max            -> x / max(x)
    standarization -> min-max u [0,1]
    gaussian       -> z-score (x-mu)/sigma   (izjednaci i srednju i varijancu)
    sentinel_0.5   -> x / medijan            (robusno na outliere)
    lamp           -> LAMP layer-adaptive    (ICLR'21)

Reuse: dijeli common3 / train_baseline3 / run_experiment3 iz parent foldera.
Pise SVOJ results.txt + summary.json (NE dira exp3 izlaze).

Pokretanje:  python run_norm_experiment.py
"""
...
# parent (critereum_experiment3) na path -> reuse modula
...
# Variramo OVO; kriterij ostaje Taylor. Slobodno skrati/poredaj listu.
...
def make_taylor(norm):
    """Taylor importance s odabranim normalizerom ('none' -> None)."""
    n = None if norm == "none" else norm
    return tp.importance.TaylorImportance(normalizer=n)


def _prune_trial(norm, ratio, calib):
    """Svjezi model -> Taylor-prune na zadani channel-ratio -> broj params (za pretragu)."""
...
def find_param_ratio(norm, search_calib, target_params):
    """Binarna pretraga channel-ratija koji daje ~target_params (TOTAL)."""
    lo, hi = 0.05, 0.95
...
def main():
    ...
    # ---- baseline (COCO-mapped, bez treninga) ----
    ...
    # ---- po normalizeru (kriterij = Taylor) ----

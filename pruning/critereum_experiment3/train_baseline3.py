"""
train_baseline3.py — fine-tune COCO-pretrained fasterrcnn_mobilenet_v3_large_320_fpn
na nasem 6-klasnom skupu (lokalizacija+klasifikacija). Dijeljena petlja sluzi i za
recovery fine-tune nakon pruninga.

Early-stop na val mAP@50:95 (patience epoha). Sprema best po val mAP.

Standalone:  python train_baseline3.py
"""
...
BATCH_SIZE = 4
NUM_WORKERS = 4
PATIENCE = 3                 # early-stop: val mAP se ne poboljsa PATIENCE epoha
BASELINE_MAX_EPOCHS = 12
GRAD_CLIP = 5.0              # clip grad-norm -> sprjecava divergenciju kod detekcijskog SGD-a
WARMUP_ITERS = 500           # linearni warmup (gornja granica; ogranicen na 1 epohu)
SEED = 42


def train_loop(model, device, train_loader, val_loader, lr, max_epochs, patience, label=""):
    """Detekcijski trening; early-stop na val mAP. Vraca (best_state, stats).

    Stabilnost: linearni warmup -> cosine decay (po iteraciji) + grad clipping.
    Bez toga recovery FT prorijeđenog detektora divergira (mAP padne na 0 nakon
    1-2 epohe jer je konstantni LR=0.005 prevec agresivan za fine-tune)."""
    model.to(device)
    ...
    def lr_factor(it):
        if it < warmup_iters:                       # 0 -> 1 linearno
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for imgs, targets in train_loader:
            ...
            scaler.unscale_(optimizer)                          # za clip na pravim grad. vrijednostima

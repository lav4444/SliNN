"""
run_inloss_pruning.py — PRUNING UGRADEN U LOSS (train-time, risk-reward), L1 vs L0,
na production detektoru, pa usporedba s post-hoc rezultatima (exp3).

Protokol po varijanti (L1 i L0):
  - teacher = nepruned COCO-mapped self (zamrznut); student = isti, trenira se.
  - svaki korak: L = det_loss + KD_W*FPN_MSE + LAMBDA * SUM cost_c*penalty(gate_c)
  - gradualno (cubic) hard-maskiramo najmanje vazne gateove dok efektivni params
    ne padne na KEEP_PARAM_FRAC (=70%); reward=cost (FLOPs+params 50/50), risk=gate grad.
  - materijalizacija: ufolda gateove -> tp magnitude rez do tocno 70% params -> BN
    rekalib -> kratki recovery FT -> eval/bench/dead.

Usporedba: tablica L1 vs L0 vs (post-hoc Taylor iz ../summary.json).

Pokretanje:  python run_inloss_pruning.py
"""
...
POSTHOC_SUMMARY = HERE.parent / "summary.json"      # exp3 rezultati (za usporedbu)

MODES = ["l1", "l0"]
KEEP_PARAM_FRAC = R.KEEP_PARAM_FRAC                  # 0.70
DEVICE = R.DEVICE

# Iterativna shema: (pre-FT) -> [REZ chunk -> FT dok val mAP ne stagnira] x PRUNE_STEPS -> finalni FT
PRUNE_STEPS = 6                  # broj rezova (svaki ~ final_removed/6 params)
PATIENCE_FT = 2                  # early-stop unutar faze: val mAP ne raste PATIENCE_FT epoha -> novi rez
MAX_EP_PER_STEP = 5              # sigurnosna kapica epoha po fazi reza
PRE_FT_PATIENCE = 2              # kratki co-adapt PRIJE 1. reza (gateovi krenu razlikovati)
MAX_EP_PRE = 3
PATIENCE_FINAL = 3               # finalni oporavak strpljiviji
MAX_EP_FINAL = 10
BATCH_SIZE = 16                 # lokalni (max sigurno na 8GB; globalni batch=4 ostaje za druge exp)
NUM_WORKERS = 2                 # manje workera -> manje host-RAM prefetcha (vise loadera istovremeno)
LR_MODEL = 0.005                # skaliran za batch 16 (~sqrt pravilo od batch4/0.0025)
LR_GATE = 0.2                   # skaliran za batch 16
KD_WEIGHT = 1.0
LAMBDA = 0.1                     # jacina cost-weighted sparsity penala
GRAD_CLIP = 5.0
WARMUP_ITERS = 500
POST_FT_EPOCHS = 8               # recovery FT nakon tp materijalizacije (early-stop, TB patience 3)
EVAL_RECALIB_BATCHES = max(4, 128 // BATCH_SIZE)   # ~128 slika za BN rekalib prije in-training eval
MON_VAL_IMAGES = 400             # mali val podskup za in-training/post-FT monitor (brze); finalni eval = pun cap
CALIB_BATCHES = max(8, 256 // BATCH_SIZE)          # ~256 slika (drzimo ih u RAM-u, ne-pinned)
SAVE_MODELS = True
...
def plot_results(summary, posthoc):
    """Two subplots (L1 | L0): #params (M) vs val mAP@[.5:.95]; training trajectory
    (soft-prune) + final materialized point + references (baseline, post-hoc Taylor)."""
...
def build_fixed_train_loader():
    """Train loader s FIKSNIM redoslijedom batcheva (shuffle jednom, pa shuffle=False).
    Nuzno za cache teacher feature-a: isti batch (isti padding) svaku epohu -> FPN
    dimenzije se poklapaju s cachiranim teacherom."""
...
def eval_with_bn_recalib(model, gc, calib_small, val_loader):
    """Rekalibriraj BN (s aktivnim gate maskama) pa evaluiraj. NUZNO jer gate mnozi
    conv izlaz PRIJE BN-a -> nakon maskiranja stare running-stats vise ne odgovaraju
    (eval bi davao garbage / mAP=0). Cuva i vraca momentum da ne pokvari trening."""
    gc.eval()
    moms = {}
...
def train_variant(mode, teacher, student, gc, kd, train_loader, val_loader, total_params, calib_small):
    ...
    st = {"it": 0, "epoch": 0}
    history = []
    teacher_cache = []               # per-batch fp16 CPU FPN feats (1. epoha racuna, dalje cita)

    def lr_scale(i):
        return (i + 1) / warmup_iters if i < warmup_iters else 1.0   # warmup -> konstantno (broj epoha nije fiksan)
        ...
        for bi, (imgs, targets) in enumerate(train_loader):
            ...
            if bi < len(teacher_cache):
                ...
                with torch.no_grad():
                    ...
                    teacher.backbone(t_imgs.tensors)      # backbone+FPN -> kd.t_feat (hook)
                t_feat = kd.t_feat
    ...
    def ft_until_plateau(phase, patience, max_ep, track_best=False):
        """FT dok val mAP ne stagnira `patience` epoha (ili max_ep). Vrati (best_state, best_map)."""
    ...
    # 0) kratki co-adapt prije 1. reza (gateovi krenu razlikovati vazno/nevazno)
    ft_until_plateau("pre", PRE_FT_PATIENCE, MAX_EP_PRE)
    # 1) iterativno: REZ chunk -> FT dok val mAP ne stagnira (PATIENCE_FT) -> novi rez
    ...
    # 2) finalni oporavak (strpljiviji), cuvamo najbolji za materijalizaciju
...
# =========================== MATERIJALIZACIJA (tp rez do 70%) =========================== #
def materialize(student, gc, target_frac):
    gc.fold_into_weights()                 # ufolda mask*gate u tezine
    gc.remove()                            # makni hookove
    student.eval()
...
def main():
    ...
    loaders = {
        "ft_train": build_fixed_train_loader(),     # fiksni redoslijed (za teacher-feature cache)
    ...
    # calib: lean (bez workera, ne-pinned) da ne raste host RAM (drzimo original-rez slike)
    ...
    # baseline (referenca) — nepruned COCO-mapped
    ...
    # post-hoc (exp3) Taylor za usporedbu
    posthoc = {}

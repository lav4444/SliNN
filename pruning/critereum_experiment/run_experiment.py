"""
run_experiment.py — usporedba 4 kriterija vaznosti za STRUCTURED pruning.

Kriteriji:  magnitude | gradient | taylor | hessian(dijag, OBD/Fisher)

Za SVAKU metodu (identican protokol):
  1) izracun vaznosti kanala  + mjeri OVERHEAD (vrijeme, GPU mem, /slika)
  2) rezanje do istog budzeta parametara (zadrzi KEEP_PARAM_FRAC, default 20% -> izbaci 80%)
  3) evaluacija ODMAH NAKON rezanja, BEZ fine-tunea:
        (a) sirovo (zastarjeli BN)         [opcionalno]
        (b) nakon kratke BN rekalibracije  [pravedno]
     na train/val/test splitu
  4) fine-tune (PONOVNI trening), isti hiperparametri kao originalni KD trening
  5) finalna evaluacija (train/val/test) + benchmark brzine (CPU/GPU)

Mjeri i: broj parametara, FLOPs, velicina; te NORMALIZIRANE/usporedive metrike
overhead-a (ms/slika za vaznost, s/epoha za fine-tune).

Rezultat: results.txt  (BEZ trening logova).

VAZNO: model StudentYOLO je cisto konvolucijski -> "neuron" == izlazni conv
filter/kanal; pruning_lib podrzava i nn.Linear neurone radi ponovne upotrebe.

Pokretanje (rucno, env dipl):
    conda activate dipl
    cd /home/tomi/code/dipl/pruning/critereum_experiment
    python run_experiment.py

NAPOMENA O TRAJANJU: pun eksperiment je tezak (4 metode x evaluacije na 3
splita x fine-tune). Za brzi smoke-test smanji FT_MAX_EPOCHS i CALIB_BATCHES,
ili iskljuci DO_RAW_EVAL / neke splitove nize.
"""
...
# ----- putanje do postojeceg koda (NE mijenjamo ga, samo uvozimo) -----
...
KEEP_PARAM_FRAC = 0.50      # zadrzi 50% params -> izbaci 50%
MIN_KEEP = 2                # minimalno kanala po sloju (sprijeci kolaps sloja)
ALLOC = "global"            # globalni rang + per-layer L2 normalizacija

CALIB_BATCHES = 32          # batcheva za vaznost (grad/taylor/hessian) i BN rekalib.
DO_RAW_EVAL = False         # (preskoceno) eval sa zastarjelim BN prije rekalibracije
DO_RECALIB_EVAL = True      # eval nakon BN rekalibracije (pravedni no-FT broj)

EVAL_SPLITS = ("val", "test")    # bez train splita (brze)
MAX_EVAL_IMAGES = 600            # cap slika po splitu u evaluaciji/benchmarku (None = sve)
DO_BASELINE_EVAL = False         # preskoci baseline eval (referenca je pure_KD/eval_result.txt)

# Fine-tune (oporavak) nakon rezanja: kratko + nizi LR (NE trenira od nule -
# nastavlja od prunanih istreniranih tezina). Early-stop kao u train_kd.
FT_MAX_EPOCHS = 5           # kratki recovery (umjesto originalnih 100)
FT_LR = 1e-4                # nizi LR za fine-tune (original je bio 1e-3)

SAVE_MODELS = True          # spremi finalni (prunan+FT) model po metodi

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Ogranici broj slika po splitu u evaluaciji i benchmarku (ne dira evaluate_student.py).
# EV.eval_split / benchmark interno zovu list_images(); presrecemo tu funkciju.
...
# kratki konceptualni opis slozenosti (za izvjestaj)
COMPLEXITY_NOTE = {
...
def fresh_baseline_model():
    """Ucitaj svjezi StudentYOLO s originalnim (najboljim) tezinama."""
...
def make_calib_batches(n_batches):
    """Materijaliziraj fiksni skup KD batcheva (iste podatke za sve metode)."""
...
def kd_loss_fn(model, batch):
    """loss_fn za compute_importance: vrati (scalar_loss, batch_size). fp32."""
...
def eval_all_splits(model):
    """Eval na svim EVAL_SPLITS; vrati dict split-> metrike (preko EV.eval_split)."""
    model.eval()
    out = {}
...
# =========================== FINE-TUNE =========================== #
def fine_tune(model):
    """Ponovni trening prunanog modela — isti hiperparametri kao originalni KD
    (AdamW, cosine+warmup, AMP, grad clip, early-stop na val mAP, LR reduce).
    Vraca (best_state, ft_stats). Ne pise nikakve logove na disk."""
...
def main():
    ...
    # za zajednicke kalibracijske batcheve (iste podatke za sve metode)
    ...
    for method in METHODS:
        ...
        # po-slojni prikaz koliko je zadrzano

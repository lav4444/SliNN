...
def materialize_to(student, target_gflops):
    ...
    lo, hi, best_r, best_d = 0.0, 0.95, 0.0, float("inf")
...
def run_variant(name, use_feature, use_logit, train_loader, mon_val, eval_loaders, calib, base_params, base_gflops):
    ...
    for epoch in range(1, TOTAL_EPOCHS + 1):
        ...
        if epoch <= PRUNE_EPOCHS:
            target_frac = (1.0 - KEEP_FINAL) * epoch / PRUNE_EPOCHS
            ...
            gate_pool = 1.5 * ((1.0 - KEEP_FINAL) / PRUNE_EPOCHS) * total_macs
            gc.prune_by_external_to_removed(score, gate_pool, metric="macs")

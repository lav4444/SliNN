"""
run_pipeline_pruning.py (pipeline_experiment1) — PRUNING, A-PARADIGMA, PURE-KD (BEZ GT).
Usporedba: pure LOGIT-KD vs pure FEATURE+LOGIT-KD (sve ostalo fiksno).
...
PURE-KD (bez GT, bez det lossa):
  ...
  * Zato je LOGIT u OBJE varijante; feature-only nema smisla (glave bez signala).
  Studentovi izlazi se vade RUCNO (student.transform + backbone + roi_heads), bez targeta.
...
# (ime, use_feature, use_logit)  — logit je u obje (glave); feature samo u drugoj
KD_VARIANTS = [("featlogit", True, True), ("logit", False, True)]
DEVICE = R.DEVICE
...
class KD:
    ...
    def __init__(self, teacher, use_feature, use_logit, T=1.0, box_w=1.0, w_feat=1.0):
        self.teacher = teacher; self.use_feature = use_feature; self.use_logit = use_logit
        self.T = T; self.box_w = box_w; self.w_feat = w_feat
    ...
    def loss(self, student, imgs, td):
        """RUCNI studentov forward (s gradom) -> feature MSE i/ili logit KL+L1. Vrati (total, info)."""
...
def run_variant(name, use_feature, use_logit, train_loader, mon_val, eval_loaders, calib, base_params, base_gflops):
    append_block("=" * 80)
    tag = ("feature+logit" if use_feature else "logit-only")
    ...
    kd = KD(teacher, use_feature, use_logit, T=LOGIT_T, box_w=LOGIT_BOX_W, w_feat=W_FEAT)
    ...
    for epoch in range(1, TOTAL_EPOCHS + 1):
        ...
        for bi, (imgs, _targets) in enumerate(train_loader):
            ...
            rf += info.get("feat", 0.0); rl += info.get("logit", 0.0); nb += 1
            gstep += 1
        ...
        append_block(f"  [ep{epoch:2d}/{TOTAL_EPOCHS} {phase:13s}] feat={rf/nb:.3f} logit={rl/nb:.3f} "
...
def main():
    ...
    append_block("=" * 80)
    append_block("PRUNING pipeline_experiment1 — A-PARADIGMA, PURE-KD (BEZ GT) | pure logit vs pure feat+logit")
    ...
    append_block(f"Rez: izbaci {(1-KEEP_FINAL)*100:.0f}% GFLOPs, ~3%/ep kroz {PRUNE_EPOCHS} ep (budzet u GFLOPs), konsolidacija do {TOTAL_EPOCHS} ep | "
                 f"SGD lr={LR_MODEL}+cosine | kriterij=gradient |d(KD)/dgate| | logit T={LOGIT_T} box_w={LOGIT_BOX_W} w_feat={W_FEAT}")
    append_block(f"Varijante (pure-KD, bez GT): {', '.join(n for n,_,_ in KD_VARIANTS)}")
    append_block("")
    ...
    for name, uf, ul in KD_VARIANTS:
        print(f"\n>>> KD: {name}")
        summary[name] = run_variant(name, uf, ul, train_loader, mon_val, eval_loaders, calib, base_params, base_gflops)

    append_block("=" * 80)
    append_block("USPOREDBA — pure-KD: logit-only vs feature+logit")
    append_block("=" * 80)
    ...
    (HERE / "summary.json").write_text(json.dumps(
        {k: ({kk: vv for kk, vv in v.items() if kk != "history"} if isinstance(v, dict) else v)
         for k, v in summary.items()} | {"history": {n: summary[n]["history"] for n, _, _ in KD_VARIANTS}}, indent=2))
...
def plot_compare(summary):
    ...
    ax.set_title("A-paradigm pure-KD pruning — logit-only vs feature+logit")

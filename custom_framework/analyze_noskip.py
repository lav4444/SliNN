"""
Analiza mreze NoSkipNet  (READ-ONLY).

Ova skripta NISTA ne mijenja i NISTA ne sprema na disk. Samo ucitava vec
spremljene tezine (results/NoSkipNet.pt) i radi analizu:

  1) Vizualizira konvolucijske filtere (ukupno 3) + ispise njihove vrijednosti.
  2) Analizira tezine neurona po iznosu (graf).
  3) Vizualizira svaki neuron i ispise/napise:
        - mean weight in      (prosjek ulaznih tezina)
        - mean abs weight in  (prosjek apsolutnih ulaznih tezina)

Pokretanje:  python analyze_noskip.py
(Vizualizacije se prikazuju na ekran preko plt.show(); ne spremaju se datoteke.)
"""
...
# NAPOMENA: import skip_model_versus na vrhu poziva matplotlib.use("Agg")
# (headless backend za spremanje slika u trening skripti). Zato ga importamo
# PRIJE pa onda silom vracamo interaktivni backend da plt.show() moze crtati.
...
def plot_conv_filters(filters):
    ...
    for ax, (name, f) in zip(axes, filters):
        ...
        # ispisi vrijednost u svaku celiju
...
# --------------------------------------------------------------------------- #
# 2) + 3) Analiza tezina neurona (linearni slojevi)
# --------------------------------------------------------------------------- #
def collect_neurons(model):
    """Za svaki neuron (izlazna jedinica linearnog sloja) vrati statistiku
    njegovih ULAZNIH tezina."""
    neurons = []
...
def plot_neuron_magnitudes(neurons):
    """Graf: tezine neurona po iznosu (mean |w| in) + prosjek (mean w in)."""
...
def plot_neurons_per_layer(neurons, layer_name, ncols):
    """Vizualizira svaki neuron sloja: bar njegovih ulaznih tezina +
    ispisani mean w in i mean |w| in u naslovu."""
...
def main():
    ...
    plot_neuron_magnitudes(neurons)              # graf po iznosu

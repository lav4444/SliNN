"""
Head-to-head: model SA skip connectionom vs model BEZ skip connectiona.

Trenira oba modela jedan po jedan na Fashion-MNIST (train/val/test split),
istom trening pipelineom na CUDA GPU-u. Nakon treninga mjeri za svaki model:
  - performanse (test accuracy / loss)
  - broj parametara i velicinu u memoriji
  - brzinu inferencije na GPU i CPU (2 warmup-a + 3 mjerene inferencije -> prosjek)

Na kraju sprema:
  - results/training_curves.png       (krivulje treninga, head-to-head)
  - results/head_to_head_metrics.png  (usporedba metrika, head-to-head)
  - results/comparison.txt            (tekstualna head-to-head usporedba)
"""
...
# --------------------------------------------------------------------------- #
SEED          = 42
MAX_EPOCHS    = 500          # samo sigurnosna kapica; efektivno "bez limita"
PATIENCE      = 3            # early stop: val ne napreduje 3 uzastopne epohe
BATCH_SIZE    = 128
LR            = 1e-3
SPLIT         = (0.6, 0.2, 0.2)   # train / val / test (od ukupno 70k)
BENCH_BATCH   = 256          # batch za mjerenje brzine inferencije
BENCH_WARMUP  = 2
BENCH_RUNS    = 3

HERE       = os.path.dirname(os.path.abspath(__file__))
DATA_ROOT  = os.path.join(HERE, "data")        # Fashion-MNIST se sprema ovdje
...
class SkipNet(nn.Module):
    """Model SA skip connectionom (paralelna grana iz conv1 ide ravno u klasifikator)."""
    ...
    def __init__(self):
        ...
        # combine fc1 + fc_parallel - concat into 5d vector
...
class NoSkipNet(nn.Module):
    """Model BEZ skip connectiona: isti backbone, ali paralelna grana je uklonjena.

    Klasifikator vidi samo glavni put (3 znacajke)."""
...
def make_loaders(seed):
    """Vraca (train_loader, val_loader, test_loader) s deterministicnim splitom/redom.

    Spaja originalni train (60k) + test (10k) = 70k i dijeli na 60/20/20.
    Isti pipeline za oba modela -> zovemo s istim seedom prije svakog treninga."""
    tfm = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.2860,), (0.3530,)),   # Fashion-MNIST statistike
    ])
    ...
    full = torch.utils.data.ConcatDataset([train_part, test_part])   # 70 000
...
def train_model(model, name, device):
    """Isti trening pipeline za svaki model. Vraca (best_model, history).

    Bez fiksnog broja epoha: early stopping kad val loss ne napreduje
    PATIENCE uzastopnih epoha. Vraca tezine iz najbolje (najnize val loss) epohe."""
    print(f"\n{'='*60}\nTreniram: {name}\n{'='*60}")
    set_seed(SEED)
    ...
    # vrati najbolji model (po val loss)
...
def benchmark_inference(model, device, batch=BENCH_BATCH,
    ...
    """Prosjecno vrijeme jedne inferencije (jedan batch) na zadanom uredaju.

    2 warmup-a + `runs` mjerenih -> prosjek."""
...
def plot_training_curves(results, path):
    ...
    for name, r in results.items():
        ...
        # oznaci epohu odabranih (najboljih) tezina
...
def save_weights(model, name, history):
    """Sprema tezine na disk: cisti state_dict + puni checkpoint s metapodacima."""
    sd_path   = os.path.join(OUT_DIR, f"{name}.pt")
    ckpt_path = os.path.join(OUT_DIR, f"{name}_ckpt.pt")

    # samo tezine (preporuceni nacin za prijenos/inferenciju)
    torch.save(model.state_dict(), sd_path)

    # puni checkpoint: tezine + sve sto treba za reprodukciju/odabir
    torch.save({

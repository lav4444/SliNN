"""
analyze_gui.py — lijepi, lokalni HTML dashboard za interpretaciju analize modela.

Reuse-a `analyze.py` (ista konfiguracija, isti `run()` -> i dalje pise
report/analysis_report.txt + report/layers.png), pa od strukturiranih podataka
slozi self-contained `report/report.html`:
  * tip izlaza (objasnjeno ljudski) + badge-evi
  * stat-kartice (params, GFLOPs, slojevi, filteri/neuroni, CPU/GPU brzina)
  * auto-uvidi (gdje je koncentriran racun, najteži sloj, GPU speedup...)
  * interaktivni per-layer bar-graf (GFLOPs / params / filteri), hover tooltip
  * kumulativna krivulja racuna
  * sortabilna + pretraziva tablica slojeva
  * ugradjena layers.png slika i sirovi report.txt (collapsible)

HTML je SAMODOSTATAN (podaci + slika u base64 inline) -> otvori se duplim klikom
u bilo kojem pregledniku, bez servera i bez interneta.

Postavke se zadaju u analyze.py (blok POSTAVI OVDJE). Pokretanje:
    conda activate dipl && python analyze_gui.py
"""
...
# --- dubinska analiza (INFORMATIVNO; bez stvarnog pruninga/growinga) ---
DEEP_ANALYSIS   = True    # prolaz kroz SVE slike: mrtve jedinice + pruning/growing potencijal (gradient/GradMax)
GRAD_MAX_IMAGES = 512     # cap slika za gradijentnu vaznost (mrtve jedinice koriste SVE slike)


# --------------------------------------------------------------------------- #
# Auto-uvidi (interpretacija brojki)
# --------------------------------------------------------------------------- #
def build_insights(data):
    ...
    # gustoća računa: GFLOPs po milijunu parametara
...
# --------------------------------------------------------------------------- #
# HTML predlozak (placeholderi se zamjenjuju; bez f-stringa zbog {} u CSS/JS)
# --------------------------------------------------------------------------- #

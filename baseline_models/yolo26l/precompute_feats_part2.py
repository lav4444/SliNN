"""
precompute_feats.py — TEACHER (yolo26l) neck feature-mape za feature-KD, kako ih radi
morphology pipeline za yolo: tap = ULAZI u Detect glavu (neck izlazi), egzaktni, BEZ PCA.

Tap-širine (yolo26l): P3=256@80, P4=512@40, P5=512@20. Spremamo fp16 po slici:
    <DATASET_ROOT>/yolo26l/train/feat/<stem>.pt  ->  {"feat": [t3, t4, t5]}  (~5.3 MB/img)

Preprocesiranje je IDENTIČNO student/evaluate.py letterboxu (640, boja 114, RGB, /255) pa su
feature-mape spatial-poravnate sa student ulazom i s postojećim soft-response targetima.

Response targeti (boxes_xywh, class_probs) se NE recomputiraju — već postoje u .../soft/<stem>.pt.
Feature se koristi SAMO u treningu (ne u validaciji), pa precompute ide samo za train split.

Resumable: preskače slike koje već imaju feat/<stem>.pt.

Run:
    conda activate dipl
    python precompute_feats.py
"""
...
SPLIT = "train"                       # feature se koristi samo u treningu (val = response+mAP, kao KD_GT_FGD)
IMG_SIZE = 640
BATCH = 8                             # teacher forward (no_grad) — sigurno na 8GB
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
EXPECTED = [(256, 80, 80), (512, 40, 40), (512, 20, 20)]   # P3, P4, P5 (sanity)


# ===== Letterbox (identično evaluate.py / student preprocess_image) =====
...
def main():
    ...
    # pre-hook na glavu: ulaz glave = lista neck izlaza [P3, P4, P5]
    grab = {}
    ...
    try:
        for i in range(0, len(todo), BATCH):
            ...
            grab.clear()
            model.model(batch)
            feats = grab["f"]                                   # [ [B,256,80,80], [B,512,40,40], [B,512,20,20] ]

            if done == 0:                                       # sanity na prvom batchu
                got = [tuple(f.shape[1:]) for f in feats]
                assert got == EXPECTED, f"tap shapes {got} != expected {EXPECTED}"
                print(f"  tap shapes OK: {got}")

            for j, p in enumerate(chunk):
                per = [f[j].detach().cpu().half().contiguous() for f in feats]   # 3× fp16 CPU
